@extends('layouts.app', ['title' => 'Edit Setoran - Gula'])
...
<div class="ef-page">
    <nav class="ef-nav">
        <a href="{{ route('gula.setoran.show', $setoran->id) }}" class="ef-back">
    ...
    <div class="ef-hdr">
        ...
        <div>
            <h1>Edit Setoran</h1>
            <p>{{ $setoran->sales->nama ?? '-' }} &middot; {{ $setoran->tanggal->format('d M Y') }}</p>
        </div>
    </div>

    <form action="{{ route('gula.setoran.update', $setoran->id) }}" method="POST" enctype="multipart/form-data" id="editForm">
        @csrf
        @method('PUT')
        ...
        <div class="ef-card amber">
            <div class="ef-card-hdr">
                ...
                <div class="ef-card-title">Informasi Sales</div>
            </div>
            <div class="ef-card-body">
                <div class="ef-fg">
                    <label class="ef-lbl">Sales</label>
                    <input type="text" class="ef-inp readonly" value="{{ $setoran->sales->nama ?? '-' }}{{ $setoran->sales->no_kendaraan ? ' ('.$setoran->sales->no_kendaraan.')' : '' }}" readonly>
                </div>
        ...
        <div class="ef-card indigo">
            ...
            <div class="ef-card-body">
                <div class="ef-summary">
                    ...
                    <div class="ef-sum-item">
                        <div class="ef-sum-label">Total Transfer</div>
                        <div class="ef-sum-value">Rp {{ number_format($summary['total_transfer'], 0, ',', '.') }}</div>
                    </div>
                    <div class="ef-sum-item">
                        <div class="ef-sum-label">Hutang Baru</div>
                        <div class="ef-sum-value">{{ $summary['jumlah_hutang_baru'] }} (Rp {{ number_format($summary['total_hutang_baru'], 0, ',', '.') }})</div>
                    </div>
                    <div class="ef-sum-item">
                        <div class="ef-sum-label">Hutang Dibayar Tunai</div>
                        <div class="ef-sum-value">Rp {{ number_format($debtPayment['totalTunai'], 0, ',', '.') }}</div>
                    </div>
                </div>
                <div class="ef-info-box">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
                    <div><strong>Yang seharusnya disetor:</strong> Rp {{ number_format($summary['total_tunai'] + $debtPayment['totalTunai'], 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="ef-card green">
            ...
            <div class="ef-card-body">
                <div class="ef-fg">
                    ...
                    <input type="number" name="total_setor" class="ef-inp" value="{{ old('total_setor', $summary['total_tunai'] + $debtPayment['totalTunai']) }}" min="0" step="1" required>
                ...
                <div class="ef-fg">
                    ...
                    <div class="ef-upload" onclick="document.getElementById('buktiInput').click()">
                        ...
                        <div class="ef-upload-hint">JPG/PNG/WEBP, maks 5MB</div>
                        <input type="file" name="foto_bukti_setor" id="buktiInput" accept="image/jpeg,image/jpg,image/png,image/webp" onchange="previewImage(this)">
                    </div>
        ...
        <div class="ef-actions">
            <a href="{{ route('gula.setoran.show', $setoran->id) }}" class="ef-btn ef-btn-ghost">
