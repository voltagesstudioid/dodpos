<x-app-layout>
    ...
    <style>
        ...
        /* header */
        .kn-header{background:linear-gradient(135deg,#fffbeb 0%,#fef3c7 100%);border:1px solid #fde68a;border-radius:20px;padding:24px 28px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px}
        .kn-header-left{display:flex;align-items:center;gap:16px}
        .kn-header-icon{width:52px;height:52px;background:linear-gradient(135deg,#f59e0b,#d97706);border-radius:16px;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 16px rgba(245,158,11,.35)}
        .kn-header-icon svg{width:26px;height:26px;color:#fff}
        .kn-header h1{font-size:1.375rem;font-weight:800;color:#1f2937;margin:0}
        .kn-header p{font-size:.8rem;color:#92400e;margin:2px 0 0}
        .kn-date-badge{font-size:.75rem;font-weight:600;color:#92400e;background:#fff;border:1px solid #fde68a;padding:6px 14px;border-radius:10px;display:flex;align-items:center;gap:6px}
        .kn-date-badge svg{width:14px;height:14px;color:#d97706}

        /* kpi */
        .kn-kpi-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;margin-bottom:24px}
        .kn-kpi{background:#fff;border:1px solid #fde68a;border-radius:16px;padding:20px;display:flex;align-items:flex-start;gap:14px;position:relative;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.04);transition:box-shadow .2s}
        .kn-kpi:hover{box-shadow:0 4px 14px rgba(245,158,11,.1)}
        ...
        /* filter */
        .kn-filter{background:#fff;border:1px solid #fde68a;border-radius:16px;padding:16px 20px;margin-bottom:24px;box-shadow:0 1px 3px rgba(0,0,0,.04)}
        ...
        .kn-date-lbl{font-size:.78rem;color:#92400e;font-weight:600;white-space:nowrap}
        .kn-date-input{border:1.5px solid #fde68a;border-radius:12px;padding:9px 12px;font-size:.8125rem;background:#fffbeb;color:#92400e;font-weight:500;outline:none;transition:border-color .2s}
        .kn-date-input:focus{border-color:#f59e0b;box-shadow:0 0 0 3px rgba(245,158,11,.1)}
        .kn-filter select{border:1.5px solid #fde68a;border-radius:12px;padding:9px 14px;font-size:.8125rem;background:#fffbeb;color:#92400e;font-weight:500;outline:none;min-width:150px;transition:border-color .2s;cursor:pointer}
        .kn-filter select:focus{border-color:#f59e0b;box-shadow:0 0 0 3px rgba(245,158,11,.1)}
        .kn-btn-filter{background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;padding:9px 18px;font-size:.8125rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:7px;transition:opacity .2s}
        .kn-btn-filter:hover{opacity:.88}
        .kn-btn-filter svg{width:15px;height:15px}
        .kn-btn-reset{background:#fffbeb;color:#92400e;border:1.5px solid #fde68a;border-radius:12px;padding:9px 16px;font-size:.8125rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:7px;text-decoration:none;transition:background .2s}
        .kn-btn-reset:hover{background:#fef3c7}
        .kn-btn-reset svg{width:15px;height:15px}

        /* chart card */
        .kn-chart-card{background:#fff;border:1px solid #fde68a;border-radius:20px;padding:24px;margin-bottom:24px;box-shadow:0 2px 8px rgba(0,0,0,.05)}
        .kn-chart-title{font-size:.9rem;font-weight:700;color:#1f2937;margin-bottom:16px;display:flex;align-items:center;gap:8px}
        .kn-chart-title svg{width:18px;height:18px;color:#d97706}
        ...
        .kn-bar-track{flex:1;height:28px;background:#fef3c7;border-radius:10px;overflow:hidden;position:relative}
        .kn-bar-fill{height:100%;background:linear-gradient(90deg,#f59e0b,#d97706);border-radius:10px;display:flex;align-items:center;justify-content:flex-end;padding-right:10px;min-width:32px;transition:width .4s}
        .kn-bar-val{font-size:.72rem;font-weight:700;color:#fff}

        /* table */
        .kn-tbl-card{background:#fff;border:1px solid #fde68a;border-radius:20px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.05)}
        .kn-tbl-wrap{overflow-x:auto}
        .kn-tbl{width:100%;border-collapse:collapse}
        .kn-tbl thead th{background:linear-gradient(180deg,#fffbeb,#fef9ee);border-bottom:2px solid #fde68a;padding:13px 16px;font-size:.7rem;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap}
        .kn-tbl tbody td{padding:14px 16px;border-bottom:1px solid #fef3c7;font-size:.8125rem;color:#374151;vertical-align:middle}
        .kn-tbl tbody tr:last-child td{border-bottom:none}
        .kn-tbl tbody tr{transition:background .15s}
        .kn-tbl tbody tr:hover{background:#fffbeb}
        ...
        .kn-sales-av{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#f59e0b,#d97706);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:.72rem;flex-shrink:0}
        ...
        /* pagination */
        .kn-pagination{padding:16px 20px;border-top:1px solid #fde68a;background:linear-gradient(180deg,#fffbeb,#fef9ee)}

        /* empty */
        .kn-empty{text-align:center;padding:60px 24px}
        .kn-empty-icon{width:80px;height:80px;margin:0 auto 18px;background:linear-gradient(135deg,#fef3c7,#fde68a);border-radius:24px;display:flex;align-items:center;justify-content:center}
        .kn-empty-icon svg{width:38px;height:38px;color:#d97706}
    ...
    <div class="kn-wrap" style="padding:24px">
        ...
        <div class="kn-header">
            <div class="kn-header-left">
                ...
                <div>
                    ...
                    <p>Tracking kunjungan sales gula</p>
                </div>
            </div>
        ...
        <div class="kn-filter">
            <form method="GET">
                ...
                <a href="{{ route('gula.kunjungan.index') }}" class="kn-btn-reset">
                ...
                <table class="kn-tbl">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align:center">
                                <a href="{{ route('gula.kunjungan.show', $k) }}" class="kn-act">
