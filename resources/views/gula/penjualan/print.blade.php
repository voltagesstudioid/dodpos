...
<body>
    ...
    <div class="receipt-wrap" style="{{ $loop->first ? 'margin-bottom: 2rem;' : '' }}">
        <div class="receipt">
            {{-- Header --}}
            <div class="rpt-hdr">
                <div style="font-size:0.625rem; font-weight:800; border:1px solid #92400e; border-radius:4px; display:inline-block; padding:2px 8px; margin-bottom:8px; color:#92400e; letter-spacing:1px;">{{ $jenisStruk }}</div>
    ...
    <script>
        ...
        function getReceiptText(jenisStruk) {
            ...
            receipt += "Tgl   : {{ $penjualan->tanggal_jual->format('d/m/Y') }} {{ $penjualan->created_at->format('H:i') }}\n";
            receipt += "--------------------------------\n";
            ...
            if (tipeBayar === 'tunai') {
                ...
                receipt += formatLine("DP", "Rp {{ number_format($penjualan->bayar ?? 0, 0, ',', '.') }}") + "\n";
                receipt += formatLine("SISA", "Rp {{ number_format($penjualan->hutang ?? 0, 0, ',', '.') }}") + "\n";
                ...
                receipt += "Ref: {{ $penjualan->transfer_ref ?? '-' }}\n";
            }
