<x-app-layout>
    ...
    <style>
        ...
        .sc-hdr-tag { display:inline-block; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:#d97706; background:#fffbeb; padding:3px 10px; border-radius:20px; margin-bottom:6px; }
        ...
        .sc-sec-ico.amber { background:#fffbeb; color:#d97706; }
        ...
        .sc-inp:focus { border-color:#f59e0b; background:#fff; box-shadow:0 0 0 3px rgba(245,158,11,.08); }
        ...
        .sc-sel:focus { border-color:#f59e0b; background-color:#fff; box-shadow:0 0 0 3px rgba(245,158,11,.08); }
        ...
        .sc-ta:focus { border-color:#f59e0b; background:#fff; box-shadow:0 0 0 3px rgba(245,158,11,.08); }
        ...
        .sc-radio-pill input:checked + .sc-radio-face { border-color:#f59e0b; background:linear-gradient(180deg,#fffbeb,#fefce8); box-shadow:0 0 0 3px rgba(245,158,11,.1); }
        .sc-radio-pill input:checked + .sc-radio-face .lbl { color:#92400e; }
        .sc-radio-pill:hover .sc-radio-face { border-color:#fbbf24; }
        .sc-info { background:#fffbeb; border:1px solid #fde68a; border-radius:10px; padding:12px 14px; display:flex; gap:10px; align-items:flex-start; }
        .sc-info-ico { width:20px; height:20px; flex-shrink:0; color:#d97706; }
        .sc-info-txt { font-size:12px; color:#78350f; line-height:1.6; }
        .sc-info-txt strong { font-weight:700; }
        .sc-stok-box { background:#fffbeb; border:1px solid #fde68a; border-radius:10px; padding:12px 14px; margin-top:10px; }
        .sc-stok-lbl { font-size:11px; font-weight:700; color:#92400e; text-transform:uppercase; letter-spacing:.06em; }
        .sc-stok-val { font-size:1.5rem; font-weight:800; color:#d97706; font-family:'JetBrains Mono',monospace; }
        ...
        .sc-btn-submit { flex:1; display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:12px 24px; border-radius:12px; font-size:14px; font-weight:700; border:none; cursor:pointer; transition:all .25s; font-family:inherit; background:linear-gradient(135deg,#f59e0b,#d97706); color:#fff; box-shadow:0 4px 16px rgba(217,119,6,.25); }
        .sc-btn-submit:hover { transform:translateY(-1px); box-shadow:0 8px 28px rgba(217,119,6,.35); }
    ...
    @endpush
    <div class="sc-page">
        <a href="{{ route('gula.stok-masuk.index') }}" class="sc-back"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg> Kembali</a>
        ...
        <form method="POST" action="{{ route('gula.stok-masuk.store') }}" id="form-stok">@csrf
            ...
            <div class="sc-sec"><div class="sc-sec-hdr"><div class="sc-sec-ico sky"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg></div><div class="sc-sec-title">Detail Stok</div></div>
                <div class="sc-sec-body">
                    ...
                    <div class="sc-fld" style="margin-top:12px;"><label class="sc-lbl" id="lbl-jumlah">Jumlah Diterima <span class="sc-req">*</span></label><input type="number" name="jumlah" id="inp-jumlah" value="{{ old('jumlah') }}" min="1" step="1" class="sc-inp" required placeholder="Masukkan jumlah...">@error('jumlah')<div class="sc-err">{{ $message }}</div>@enderror</div>
                    <div class="sc-fld"><label class="sc-lbl">Sumber / Supplier</label><input type="text" name="sumber" value="{{ old('sumber') }}" class="sc-inp" placeholder="Contoh: PT. Madu Sari">@error('sumber')<div class="sc-err">{{ $message }}</div>@enderror</div>
            ...
            <div class="sc-submit-bar"><a href="{{ route('gula.stok-masuk.index') }}" class="sc-btn-cancel">Batal</a><button type="submit" class="sc-btn-submit"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Simpan</button></div>
        </form></div>
    @push('scripts')
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updTipe() {
            ...
            if (t==='penerimaan') { iP.style.display='flex'; iK.style.display='none'; lbl.innerHTML='Jumlah Diterima <span class="sc-req">*</span>'; inp.placeholder='Berapa jumlah yang diterima...'; }
            else { iP.style.display='none'; iK.style.display='flex'; lbl.innerHTML='Stok Aktual (Fisik) <span class="sc-req">*</span>'; inp.placeholder='Berapa stok fisik saat ini...'; }
        }
        sel.addEventListener('change', function() { var o=this.options[this.selectedIndex]; if(o&&o.value){sBox.style.display='block';sVal.textContent=fmt(parseInt(o.dataset.stok)||0)+' '+(o.dataset.satuan||'Dus');}else{sBox.style.display='none';} });
        document.querySelectorAll('input[name="tipe"]').forEach(function(r){r.addEventListener('change',updTipe);});
        updTipe();
        if(sel.value){var o=sel.options[sel.selectedIndex];sBox.style.display='block';sVal.textContent=fmt(parseInt(o.dataset.stok)||0)+' '+(o.dataset.satuan||'Dus');}
