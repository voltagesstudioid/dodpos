<x-app-layout>
    ...
    <style>
        ...
        .sm-hdr.penerimaan { background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%); }
        .sm-hdr.koreksi { background:linear-gradient(135deg,#3b82f6 0%,#2563eb 100%); }
        ...
        .sm-sec-ico.amber { background:#fffbeb; color:#d97706; }
    ...
    @endpush
    <div class="sm-page">
        <a href="{{ route('gula.stok-masuk.index') }}" class="sm-back"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg> Kembali ke Daftar</a>
        ...
        <div class="sm-sec">
            <div class="sm-sec-hdr">
                <div class="sm-sec-ico amber"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg></div>
        ...
        <div class="sm-sec">
            ...
            <div class="sm-sec-body">
                @php
                    ...
                    $satuan = $stokMasuk->produk?->satuan ?? 'Dus';
                @endphp
                <div class="sm-flow">
                    <div class="sm-flow-box">
                        <div class="sm-flow-lbl">Stok Sebelum</div>
                        <div class="sm-flow-val blue">{{ number_format((float) $stokMasuk->stok_sebelum, 0, ',', '.') }}</div>
                    ...
                    <div class="sm-flow-box" style="border-color:{{ $diff >= 0 ? '#bbf7d0' : '#fecaca' }}; background:{{ $diff >= 0 ? '#f0fdf4' : '#fef2f2' }};">
                        ...
                        <div class="sm-flow-val {{ $diff >= 0 ? 'green' : 'red' }}">{{ $diffSign }}{{ number_format($diff, 0, ',', '.') }}</div>
                    ...
                    <div class="sm-flow-box">
                        ...
                        <div class="sm-flow-val green">{{ number_format((float) $stokMasuk->stok_sesudah, 0, ',', '.') }}</div>
                        <div style="font-size:10px;color:#94a3b8;">{{ $satuan }}</div>
                    </div>
                </div>
            </div>
        ...
        <div class="sm-actions">
            <a href="{{ route('gula.stok-masuk.index') }}" class="sm-btn sm-btn-outline">
            ...
            <form method="POST" action="{{ route('gula.stok-masuk.cancel', $stokMasuk) }}" onsubmit="return confirm('Yakin ingin membatalkan data ini? Stok akan disesuaikan kembali.');" style="flex:1;">
                @csrf
