<x-app-layout>
    ...
    <style>
        ...
        .fm-kode-badge { display:inline-flex; align-items:center; gap:0.375rem; padding:0.25rem 0.625rem; border-radius:8px; background:linear-gradient(135deg,#eff6ff,#dbeafe); font-size:0.75rem; font-weight:700; color:#2563eb; font-family:'JetBrains Mono',monospace; letter-spacing:-0.01em; margin-top:0.375rem; }
    ...
    <div class="py-4">
        <div class="fm-page">
            ...
            <div class="fm-hdr">
                ...
                <div>
                    <div class="fm-hdr-title">Edit Produk Mineral</div>
                    <div class="fm-hdr-sub">Ubah data produk mineral</div>
                    <div class="fm-kode-badge">
            ...
            <form method="POST" action="{{ route('mineral.produk.update', $produk) }}" id="produkForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="fm-section-body">
                        <div class="fm-grid fm-grid-2">
                            <div class="fm-full">
                                ...
                                <input type="text" name="nama" value="{{ old('nama', $produk->nama) }}" required placeholder="Contoh: Aqua Galon 19L, Le Minerale 600ml" class="fm-input" autocomplete="off">
                            ...
                            <div>
                                ...
                                <select name="jenis" class="fm-input">
                                    ...
                                    @php $jenisVal = old('jenis', $produk->jenis); @endphp
                                    @foreach($jenisList as $j)
                                        <option value="{{ $j->nama }}" {{ $jenisVal == $j->nama ? 'selected' : '' }}>{{ $j->nama }}</option>
                                    @endforeach
                                    @if($produk->jenis && !$jenisList->contains('nama', $produk->jenis))
                                        <option value="{{ $produk->jenis }}" selected>{{ $produk->jenis }} (arsip)</option>
                                    @endif
                                </select>
                            ...
                            <div>
                                ...
                                <select name="satuan" required class="fm-input">
                                    ...
                                    @php $satuanVal = old('satuan', $produk->satuan); @endphp
                                    @foreach($satuanList as $s)
                                        <option value="{{ $s->nama }}" {{ $satuanVal == $s->nama ? 'selected' : '' }}>{{ $s->nama }}@if($s->singkatan) ({{ $s->singkatan }})@endif</option>
                                    @endforeach
                                    @if($produk->satuan && !$satuanList->contains('nama', $produk->satuan))
                                        <option value="{{ $produk->satuan }}" selected>{{ $produk->satuan }} (arsip)</option>
                                    @endif
                                </select>
                        ...
                        <div class="fm-grid fm-grid-2">
                            <div>
                                ...
                                <div class="fm-input-wrap">
                                    ...
                                    <input type="text" id="hargaModal" inputmode="numeric" value="{{ old('harga_modal', (int)$produk->harga_modal) }}" placeholder="0" class="fm-input has-prefix green">
                                    <input type="hidden" name="harga_modal" id="hargaModalHidden" value="{{ old('harga_modal', (int)$produk->harga_modal) }}">
                                </div>
                                <div class="fm-hint">
                            ...
                            <div>
                                ...
                                <div class="fm-input-wrap">
                                    <span class="fm-input-prefix">Rp</span>
                                    <input type="text" id="hargaJual" inputmode="numeric" value="{{ old('harga_jual', (int)$produk->harga_jual) }}" required placeholder="0" class="fm-input has-prefix green">
                                    <input type="hidden" name="harga_jual" id="hargaJualHidden" value="{{ old('harga_jual', (int)$produk->harga_jual) }}">
                                </div>
                                <div class="fm-hint">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            ...
                            <div>
                                ...
                                <div class="fm-input-wrap">
                                    <input type="number" name="stok_gudang" value="{{ old('stok_gudang', $produk->stok_gudang) }}" min="0" placeholder="0" class="fm-input has-suffix green">
                                    <span class="fm-input-suffix">{{ $produk->satuan }}</span>
                                </div>
                                <div class="fm-hint">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    Stok fisik saat ini di gudang
                                </div>
                                @error('stok_gudang')<div class="fm-error"><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>{{ $message }}</div>@enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="fm-input-wrap">
                                    <input type="number" name="stok_minimum" value="{{ old('stok_minimum', $produk->stok_minimum) }}" min="0" placeholder="10" class="fm-input has-suffix green">
                                    <span class="fm-input-suffix">{{ $produk->satuan }}</span>
                                </div>
                                <div class="fm-hint">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        ...
                        <div class="fm-grid fm-grid-2">
                            <div class="fm-full">
                                ...
                                <div class="fm-radio-group" id="statusGroup">
                                    <label class="fm-radio-card {{ $currentStatus == 'aktif' ? 'selected' : '' }}" style="--sel-color:#059669;--sel-bg:#ecfdf5;" onclick="selectRadio(this)">
                                    ...
                                    <label class="fm-radio-card {{ $currentStatus == 'nonaktif' ? 'selected' : '' }}" style="--sel-color:#64748b;--sel-bg:#f8fafc;" onclick="selectRadio(this)">
                            ...
                            <div class="fm-full">
                                ...
                                <textarea name="keterangan" rows="3" placeholder="Catatan tambahan tentang produk (opsional)" class="fm-input purple">{{ old('keterangan', $produk->keterangan) }}</textarea>
    ...
    function formatCurrency(input, hiddenInput) {
        ...
        if (initVal && /^\d+$/.test(initVal)) {
            input.value = parseInt(initVal).toLocaleString('id-ID');
        }
