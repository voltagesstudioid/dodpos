<x-app-layout>
    ...
    <style>
        ...
        .st-sel:focus { border-color:#2563eb; background-color:#fff; box-shadow:0 0 0 3px rgba(37,99,235,.08); }
    ...
    <div class="st-page">
        <a href="{{ route('mineral.setoran.index') }}" class="st-back">
        ...
        <form method="POST" action="{{ route('mineral.setoran.store') }}" id="form-setoran" enctype="multipart/form-data">
            @csrf
            <div class="st-sec">
                ...
                <div class="st-sec-body">
                    ...
                    <div class="st-fld">
                        ...
                        <select name="sales_id" class="st-sel" required>
                            ...
                            @foreach($sales as $s)<option value="{{ $s->id }}" {{ old('sales_id') == $s->id ? 'selected' : '' }}>{{ $s->nama }}</option>@endforeach
                        </select>
            ...
            <div class="st-sec">
                ...
                <div class="st-sec-body">
                    ...
                    <div class="st-fld">
                        ...
                        <input type="file" name="foto_bukti_setor" id="inp-bukti" accept="image/*" capture="environment" style="display:none;">
                        <div id="bukti-preview-wrap" style="margin-top:8px; display:none;">
                            <img id="bukti-preview" src="" alt="Preview" style="max-width:100%; max-height:160px; border-radius:10px; border:1.5px solid #6366f1;">
                        </div>
                        <button type="button" id="btn-pilih-bukti" style="display:inline-flex; align-items:center; gap:6px; padding:10px 16px; border-radius:10px; border:1.5px dashed #94a3b8; background:#f8fafc; font-size:13px; font-weight:600; color:#64748b; cursor:pointer; transition:all .2s; font-family:inherit; width:100%; justify-content:center;">
                            <span>📷</span> Ambil / Pilih Foto Bukti Setoran
                        </button>
                    </div>
                    <div class="st-info-box"><strong>Catatan:</strong> Total penjualan, jumlah transaksi, dan data hutang akan dihitung otomatis berdasarkan penjualan terverifikasi pada tanggal yang dipilih.</div>
                </div>
            </div>

            <div class="st-submit-bar">
                <a href="{{ route('mineral.setoran.index') }}" class="st-btn-cancel"><svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg> Batal</a>
                <button type="submit" class="st-btn-submit"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Simpan Setoran</button>
            </div>
        </form>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Bukti setoran upload
        var btnPilih = document.getElementById('btn-pilih-bukti');
        var inpBukti = document.getElementById('inp-bukti');
        var preview = document.getElementById('bukti-preview');
        var previewWrap = document.getElementById('bukti-preview-wrap');
