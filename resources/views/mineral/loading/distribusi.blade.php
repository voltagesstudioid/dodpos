<x-app-layout>
    ...
    <style>
        ...
        .ds-hdr { background:linear-gradient(135deg,#155e75 0%,#0891b2 40%,#06b6d4 100%); border-radius:20px; padding:1.75rem; margin-bottom:1.5rem; box-shadow:0 12px 40px rgba(8,145,178,.25); position:relative; overflow:hidden; }
        ...
        .ds-card-hdr { padding:1rem 1.25rem; border-bottom:1px solid #e0f7fa; display:flex; align-items:center; gap:.625rem; background:linear-gradient(180deg,#ecfeff,#f8fffe); }
        ...
        .ds-card-ico.cyan { background:linear-gradient(135deg,#cffafe,#a5f3fc); color:#155e75; }
        ...
        .ds-inp:focus { border-color:#06b6d4; box-shadow:0 0 0 3px rgba(6,182,212,.12); }
        ...
        .ds-sel:focus { border-color:#06b6d4; box-shadow:0 0 0 3px rgba(6,182,212,.12); }
        ...
        .ds-ta:focus { border-color:#06b6d4; box-shadow:0 0 0 3px rgba(6,182,212,.12); }
        .ds-grid { display:grid; grid-template-columns:1fr 1fr; gap:1rem; }
        @media(max-width:640px) { .ds-grid { grid-template-columns:1fr; } }
        .ds-stok { background:#ecfeff; border:1px solid #a5f3fc; border-radius:12px; padding:1rem 1.25rem; display:flex; align-items:center; justify-content:space-between; }
        .ds-stok-lbl { font-size:11px; font-weight:700; color:#155e75; text-transform:uppercase; letter-spacing:.06em; }
        .ds-stok-val { font-size:1.5rem; font-weight:800; color:#0891b2; font-family:'JetBrains Mono',monospace; }
        .ds-stok-unit { font-size:.75rem; font-weight:600; color:#67e8f9; }
        ...
        .ds-row-num { position:absolute; top:-8px; left:12px; background:linear-gradient(135deg,#06b6d4,#0891b2); color:#fff; font-size:10px; font-weight:800; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 6px rgba(8,145,178,.3); }
        ...
        .ds-add { display:flex; align-items:center; justify-content:center; gap:8px; width:100%; padding:.75rem; border-radius:12px; border:2px dashed #0891b2; background:#ecfeff; color:#155e75; font-size:13px; font-weight:700; cursor:pointer; transition:all .2s; font-family:inherit; }
        .ds-add:hover { background:#cffafe; border-color:#0e7490; }
        .ds-sum { background:linear-gradient(135deg,#ecfeff,#cffafe); border:1px solid #a5f3fc; border-radius:12px; padding:1.125rem 1.25rem; }
        .ds-sum-title { font-size:11px; font-weight:700; color:#155e75; text-transform:uppercase; letter-spacing:.06em; margin-bottom:.75rem; }
        .ds-sum-row { display:flex; justify-content:space-between; align-items:center; padding:4px 0; font-size:13px; }
        .ds-sum-key { color:#155e75; font-weight:500; }
        .ds-sum-val { color:#164e63; font-weight:700; font-family:'JetBrains Mono',monospace; font-size:14px; }
        .ds-sum-val.danger { color:#dc2626; }
        .ds-sum-val.ok { color:#059669; }
        .ds-sum-divider { border-top:1px dashed #a5f3fc; margin:6px 0; }
        ...
        .ds-btn-submit { flex:1; display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:.9375rem 1.5rem; border-radius:14px; font-size:.9375rem; font-weight:700; border:none; cursor:pointer; transition:all .25s; font-family:inherit; background:linear-gradient(135deg,#06b6d4,#0891b2); color:#fff; box-shadow:0 8px 24px rgba(8,145,178,.3); }
        .ds-btn-submit:hover { transform:translateY(-2px); box-shadow:0 12px 36px rgba(8,145,178,.4); }
    ...
    @endpush
    <div class="ds-page">
        <a href="{{ route('mineral.loading.index') }}" class="ds-back"><svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg> Kembali</a>
        ...
        <form method="POST" action="{{ route('mineral.loading.distribusi.store') }}" id="form-dist">
            @csrf

            {{-- Tanggal & Produk --}}
            <div class="ds-card">
                <div class="ds-card-hdr">
                    <div class="ds-card-ico cyan"><svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg></div>
                ...
                <div class="ds-card-body">
                    <div class="ds-grid">
                        ...
                        <div class="ds-fld">
                            <label class="ds-lbl">Produk Mineral <span class="ds-req">*</span></label>
                    ...
                    <div class="ds-stok" id="stok-box" style="display:none;">
                        <div><div class="ds-stok-lbl">Stok Gudang Tersedia</div><div style="font-size:11px;color:#67e8f9;margin-top:2px;" id="stok-produk-name"></div></div>
                        <div style="text-align:right;"><div class="ds-stok-val" id="stok-val">0</div><div class="ds-stok-unit" id="stok-unit">Liter</div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="ds-card">
                ...
                <div class="ds-card-body">
                    <div class="ds-sum">
                        ...
                        <div class="ds-sum-row"><span class="ds-sum-key">Stok Gudang</span><span class="ds-sum-val" id="sum-stok">0 Liter</span></div>
                        <div class="ds-sum-row"><span class="ds-sum-key">Total Distribusi (<span id="sum-count">0</span> sales)</span><span class="ds-sum-val" id="sum-total">0 Liter</span></div>
                        <div class="ds-sum-divider"></div>
                        <div class="ds-sum-row"><span class="ds-sum-key" style="font-weight:700;">Sisa Stok Gudang</span><span class="ds-sum-val" id="sum-sisa">0 Liter</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="ds-bar">
                <a href="{{ route('mineral.loading.index') }}" class="ds-btn-cancel">Batal</a>
    ...
    function addTankerRow() {
        ...
        var html = '<div class="ds-row" id="row-' + idx + '">' +
            ...
            '<div class="ds-row-grid">' +
                ...
                '<div class="ds-fld" style="margin-bottom:0"><label class="ds-lbl">Volume</label>' +
    ...
    document.getElementById('sel-produk').addEventListener('change', function() {
        ...
        if (opt && opt.value) {
            ...
            currentSatuan = opt.dataset.satuan || 'Liter';
