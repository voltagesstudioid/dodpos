    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            @php
                ...
                $isCancelled = $kunjungan->status === 'cancel';
                $durasi = $isCompleted ? $kunjungan->waktu_checkin->diffInMinutes($kunjungan->waktu_checkout) : null;
            @endphp
            <div class="mb-6 p-4 rounded-2xl {{ $isCancelled ? 'bg-red-50 border border-red-200' : ($isCompleted ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200') }}">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-xl {{ $isCancelled ? 'bg-red-100' : ($isCompleted ? 'bg-green-100' : 'bg-amber-100') }} flex items-center justify-center">
                        <svg class="w-6 h-6 {{ $isCancelled ? 'text-red-600' : ($isCompleted ? 'text-green-600' : 'text-amber-600') }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            @if($isCancelled)
                    ...
                    <div>
                        <div class="font-semibold {{ $isCancelled ? 'text-red-800' : ($isCompleted ? 'text-green-800' : 'text-amber-800') }}">
                            @if($isCancelled)
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        @if($kunjungan->jarak_checkin !== null)
                        <div class="flex justify-between py-2 border-b border-gray-50">
                            <span class="text-sm text-gray-500">Jarak dari Toko</span>
                            <span class="text-sm font-medium {{ $kunjungan->jarak_checkin > 500 ? 'text-red-600' : 'text-green-600' }}">
                                {{ number_format($kunjungan->jarak_checkin) }} m
                                @if($kunjungan->jarak_checkin > 500)
                                    <span class="text-xs">(Jauh)</span>
                                @endif
                            </span>
                        </div>
                        @endif
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between py-2 border-b border-gray-50">
                            ...
                            <a href="https://maps.google.com/?q={{ $kunjungan->latitude_checkout }},{{ $kunjungan->longitude_checkout }}" 
                               target="_blank" class="text-sm font-medium text-blue-600 hover:underline">
                                {{ number_format((float)$kunjungan->latitude_checkout, 6) }}, {{ number_format((float)$kunjungan->longitude_checkout, 6) }}
                            </a>
                        </div>
                        @if($kunjungan->jarak_checkout !== null)
                        <div class="flex justify-between py-2 border-b border-gray-50">
                            <span class="text-sm text-gray-500">Jarak dari Toko</span>
                            <span class="text-sm font-medium {{ $kunjungan->jarak_checkout > 500 ? 'text-red-600' : 'text-green-600' }}">
                                {{ number_format($kunjungan->jarak_checkout) }} m
                                @if($kunjungan->jarak_checkout > 500)
                                    <span class="text-xs">(Jauh)</span>
                                @endif
                            </span>
                        </div>
            ...
            {{-- Checkout Form — shown when visit is still active and user is sales --}}
            @if(!$kunjungan->waktu_checkout && !$isCancelled && $isSalesRole)
            <div class="mt-6 bg-white rounded-2xl shadow-sm border border-blue-200 p-6" id="checkout-section">
                <h3 class="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('mineral.kunjungan.checkout', $kunjungan) }}" enctype="multipart/form-data" id="form-checkout">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <button type="button" class="mt-3 w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors" id="co-btn-camera">
                    ...
                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl text-base font-bold bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5" id="co-btn-submit">
            ...
            <div class="mt-6 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                @if($kunjungan->ada_penjualan && $kunjungan->penjualan && !$isCancelled)
                <div class="bg-green-50 border border-green-200 rounded-xl p-4">
                    ...
                    <div class="mt-3 pt-3 border-t border-green-200">
                        <a href="{{ route('mineral.penjualan.show', $kunjungan->penjualan) }}" class="inline-flex items-center gap-1 text-sm font-medium text-green-700 hover:text-green-900">
            ...
            <div class="mt-6 flex items-center gap-4 flex-wrap">
                ...
                @if($kunjungan->status === 'checkin' && !$kunjungan->waktu_checkout && !$isCancelled)
                <form method="POST" action="{{ route('mineral.kunjungan.cancel', $kunjungan) }}" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan kunjungan ini?')">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 text-red-600 hover:text-red-800 px-4 py-3 rounded-xl font-medium transition-colors border border-red-200 hover:border-red-300 hover:bg-red-50">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @if($kunjungan->latitude_checkin && $kunjungan->longitude_checkin)
            const checkinLat = {{ (float)$kunjungan->latitude_checkin }};
            const checkinLng = {{ (float)$kunjungan->longitude_checkin }};
            ...
            L.marker([checkinLat, checkinLng]).addTo(map)
                .bindPopup('<b>Check-in</b><br>Toko: {{ $kunjungan->pelanggan->nama_toko ?? "-" }}<br>Waktu: {{ $kunjungan->waktu_checkin->format("H:i:s") }} WIB')
                .openPopup();

            // Add pelanggan location marker if available
            @if($kunjungan->pelanggan->latitude && $kunjungan->pelanggan->longitude)
            const tokoLat = {{ (float)$kunjungan->pelanggan->latitude }};
            const tokoLng = {{ (float)$kunjungan->pelanggan->longitude }};
            L.marker([tokoLat, tokoLng], {
                icon: L.divIcon({
                    html: '<div style="background:#2563eb;color:#fff;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.3);">🏪</div>',
                    iconSize: [28, 28],
                    iconAnchor: [14, 14],
                    className: ''
                })
            }).addTo(map).bindPopup('<b>Toko: {{ $kunjungan->pelanggan->nama_toko ?? "-" }}</b><br>{{ $kunjungan->pelanggan->alamat ?? "-" }}');
            @endif

            @if($kunjungan->latitude_checkout && $kunjungan->longitude_checkout)
                const checkoutLat = {{ (float)$kunjungan->latitude_checkout }};
                const checkoutLng = {{ (float)$kunjungan->longitude_checkout }};
                
                L.marker([checkoutLat, checkoutLng]).addTo(map)
                    .bindPopup('<b>Check-out</b><br>Waktu: {{ $kunjungan->waktu_checkout->format("H:i:s") }} WIB');

                const bounds = L.latLngBounds([[checkinLat, checkinLng], [checkoutLat, checkoutLng]]);
                @if($kunjungan->pelanggan->latitude && $kunjungan->pelanggan->longitude)
                    bounds.extend([tokoLat, tokoLng]);
                @endif
