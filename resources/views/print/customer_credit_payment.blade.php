...
<head>
    ...
    <style>
        ...
        .header-right h2 { margin: 0; font-size: 20px; color: #10b981; text-transform: uppercase; letter-spacing: 1px; }
...
<body onload="window.print();">
    <div class="container">
        ...
        <div class="content">
            <div class="row">
                <div class="col-label">Diterima Dari Pelanggan</div>
                <div class="col-value"><strong>{{ $kredit->customerCredit->customer->name ?? 'Pelanggan' }}</strong></div>
            </div>
            <div class="row">
                <div class="col-label">Tanggal Pembayaran</div>
                <div class="col-value">{{ \Carbon\Carbon::parse($kredit->payment_date)->format('d F Y') }}</div>
            </div>
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-value">{{ $kredit->customerCredit->credit_number }} (Trx: {{ $kredit->customerCredit->transaction_id ?? '-' }})</div>
            </div>
            <div class="row">
                <div class="col-label">Keterangan Tambahan</div>
                <div class="col-value">{{ $kredit->notes ?: '-' }}</div>
            </div>

            <div class="amount-box">
                <div class="label">Total Pembayaran</div>
                <div class="value">Rp {{ number_format($kredit->amount, 0, ',', '.') }}</div>
            </div>
        </div>

        <div class="footer">
            <div class="signature-box">
                <div class="date-line">Bandung, {{ \Carbon\Carbon::parse($kredit->payment_date)->format('d F Y') }}</div>
                <div class="signature-line">{{ $kredit->createdBy->name ?? 'Penerima / Admin' }}</div>
            </div>
        </div>
    </div>
</body>
