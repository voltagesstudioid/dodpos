<x-app-layout>
    ...
    <style>
        ...
        .sp-btn-danger{background:#fef2f2;color:#991b1b;border:1px solid #fecaca;}
    ...
    <div class="sp-cr">
        ...
        <div class="sp-top">
            <div class="sp-top-left">
                <h1>Edit Supplier</h1>
                <p>Perbarui data supplier untuk kebutuhan pembelian.</p>
            </div>
            <div class="sp-top-actions">
                ...
                <button type="submit" form="supplierEditForm" class="sp-btn sp-btn-primary">
        ...
        <form id="supplierEditForm" method="POST" action="{{ route('master.supplier.update', $supplier) }}" novalidate>
            @csrf @method('PUT')
            ...
            <div class="sp-card">
                <div class="sp-card-head">
                    ...
                    <div>
                        ...
                        <div class="sp-card-desc">Data utama supplier</div>
                    </div>
                </div>
                <div class="sp-card-body">
                    <div class="sp-grid sp-grid-2">
                        <div class="sp-field">
                            ...
                            <input type="text" name="code" value="{{ old('code', $supplier->code) }}" class="sp-input @error('code') invalid @enderror" placeholder="SUP-001">
                            @error('code') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                        <div class="sp-field">
                            <label class="sp-label">Nama Supplier / Perusahaan <span class="sp-req">*</span></label>
                            <input type="text" name="name" id="f-name" value="{{ old('name', $supplier->name) }}" class="sp-input @error('name') invalid @enderror" required>
                            @error('name') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                    </div>
                    <div class="sp-field">
                        <label class="sp-label">Kota / Wilayah</label>
                        <input type="text" name="city" value="{{ old('city', $supplier->city) }}" class="sp-input @error('city') invalid @enderror" placeholder="Bandung, Jakarta...">
                        @error('city') <div class="sp-err">{{ $message }}</div> @enderror
                    </div>
                    <div class="sp-field">
                        <label class="sp-label">Alamat Lengkap</label>
                        <textarea name="address" rows="3" class="sp-textarea @error('address') invalid @enderror" placeholder="Jl. ...">{{ old('address', $supplier->address) }}</textarea>
                        @error('address') <div class="sp-err">{{ $message }}</div> @enderror
                    </div>

                    {{-- Active toggle --}}
                    <div class="sp-field" style="margin-top:0.5rem;">
                        <input type="hidden" name="active" id="active-hidden" value="{{ old('active', $supplier->active) ? '1' : '0' }}">
                        <div class="sp-toggle {{ old('active', $supplier->active) ? 'on' : 'off' }}" id="toggle-active" onclick="toggleActive()">
            ...
            <div class="sp-card">
                <div class="sp-card-head">
                    ...
                    <div>
                        ...
                        <div class="sp-card-desc">Informasi kontak PIC</div>
                    </div>
                </div>
                <div class="sp-card-body">
                    <div class="sp-field">
                        <label class="sp-label">Kontak Person (PIC)</label>
                        <input type="text" name="contact_person" value="{{ old('contact_person', $supplier->contact_person) }}" class="sp-input @error('contact_person') invalid @enderror" placeholder="Nama PIC...">
                        @error('contact_person') <div class="sp-err">{{ $message }}</div> @enderror
                    </div>
                    <div class="sp-grid sp-grid-2">
                        <div class="sp-field">
                            <label class="sp-label">No. Telepon / WhatsApp</label>
                            <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" class="sp-input @error('phone') invalid @enderror" placeholder="08xx-xxxx-xxxx">
                            @error('phone') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                        <div class="sp-field">
                            <label class="sp-label">Email</label>
                            <input type="email" name="email" value="{{ old('email', $supplier->email) }}" class="sp-input @error('email') invalid @enderror" placeholder="nama@email.com">
                            @error('email') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                    </div>
                    <div class="sp-field">
                        <label class="sp-label">Catatan Tambahan</label>
                        <textarea name="notes" rows="3" class="sp-textarea @error('notes') invalid @enderror" placeholder="Catatan khusus supplier ini...">{{ old('notes', $supplier->notes) }}</textarea>
            ...
            <div class="sp-card">
                ...
                <div class="sp-card-body">
                    <div class="sp-field">
                        ...
                        <input type="text" name="npwp" id="f-npwp" value="{{ old('npwp', $supplier->npwp) }}" class="sp-input @error('npwp') invalid @enderror" placeholder="00.000.000.0-000.000">
                        @error('npwp') <div class="sp-err">{{ $message }}</div> @enderror
                    </div>
                    <div class="sp-grid sp-grid-3">
                        <div class="sp-field">
                            <label class="sp-label">Nama Bank</label>
                            <input type="text" name="bank_name" value="{{ old('bank_name', $supplier->bank_name) }}" class="sp-input @error('bank_name') invalid @enderror" placeholder="BCA / Mandiri...">
                            @error('bank_name') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                        <div class="sp-field">
                            <label class="sp-label">No. Rekening</label>
                            <input type="text" name="bank_account" value="{{ old('bank_account', $supplier->bank_account) }}" class="sp-input @error('bank_account') invalid @enderror" placeholder="1234567890">
                            @error('bank_account') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                        <div class="sp-field">
                            <label class="sp-label">Atas Nama Rekening</label>
                            <input type="text" name="bank_account_name" value="{{ old('bank_account_name', $supplier->bank_account_name) }}" class="sp-input @error('bank_account_name') invalid @enderror" placeholder="A.N ...">
                            @error('bank_account_name') <div class="sp-err">{{ $message }}</div> @enderror
                        </div>
                    </div>
                    <div class="sp-field">
                        <label class="sp-label">Termin Pembayaran (TOP) - Hari</label>
                        <input type="number" name="term_days" value="{{ old('term_days', $supplier->term_days) }}" min="0" class="sp-input @error('term_days') invalid @enderror" placeholder="0">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (npwp) {
            ...
            let existing = npwp.value.replace(/[^\d]/g, '');
            if (existing.length > 0) {
                existing = existing.substring(0, 15);
                let f = '';
