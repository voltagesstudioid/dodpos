<x-app-layout>
    ...
    <style>
        ...
        .sd-hdr { background:linear-gradient(135deg,#4f46e5,#6366f1); border-radius:16px; padding:1.25rem 1.5rem; margin-bottom:1.25rem; color:#fff; position:relative; overflow:hidden; }
        ...
        .sd-card-ico.indigo { background:#eef2ff; color:#4f46e5; }
        ...
        .sd-total-box { display:flex; justify-content:space-between; align-items:center; background:linear-gradient(135deg,#eef2ff,#f5f3ff); border:1px dashed #c7d2fe; border-radius:10px; padding:12px 14px; margin-top:10px; }
        .sd-total-lbl { font-size:12px; font-weight:700; color:#4f46e5; text-transform:uppercase; letter-spacing:.04em; }
        .sd-total-val { font-size:1.125rem; font-weight:800; color:#4f46e5; }
        ...
        .sd-btn-primary { background:linear-gradient(135deg,#4f46e5,#6366f1); color:#fff; box-shadow:0 3px 12px rgba(79,70,229,.2); }
        .sd-btn-primary:hover { box-shadow:0 6px 20px rgba(79,70,229,.3); transform:translateY(-1px); }
        ...
        .sd-btn-warning { background:linear-gradient(135deg,#f59e0b,#d97706); color:#fff; box-shadow:0 3px 12px rgba(217,119,6,.2); }
        .sd-btn-warning:hover { box-shadow:0 6px 20px rgba(217,119,6,.3); transform:translateY(-1px); }
    ...
    <div class="sd-page">
        <a href="{{ route('minyak.setoran.index') }}" class="sd-back">
        ...
        <div class="sd-card">
            ...
            <div class="sd-card-body">
                ...
                <div class="sd-row"><span class="sd-key">💵 Penjualan Tunai</span><span class="sd-val" style="color:#059669;">Rp {{ number_format($setoran->total_tunai ?? 0, 0, ',', '.') }}</span></div>
        ...
        <div class="sd-card">
            ...
            <div class="sd-card-body" style="text-align:center;">
                <a href="{{ asset('storage/' . $setoran->bukti_setor) }}" target="_blank" style="display:inline-block;">
                    <img src="{{ asset('storage/' . $setoran->bukti_setor) }}" alt="Bukti Setoran" style="max-width:100%; max-height:280px; border-radius:10px; border:1.5px solid #e2e8f0; cursor:pointer; transition:transform .2s;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">
                </a>
        ...
        <div class="sd-card">
            ...
            <div class="sd-card-body">
                <form method="POST" action="{{ route('minyak.setoran.verify', $setoran) }}">
                    @csrf
        ...
        <div class="sd-actions">
            <a href="{{ route('minyak.setoran.index') }}" class="sd-btn sd-btn-outline">
                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Kembali
            </a>
            @if(!auth()->user()->role || (!str_starts_with(strtolower(auth()->user()->role), 'sales_') && $setoran->status === 'pending'))
            <a href="{{ route('minyak.setoran.edit', $setoran) }}" class="sd-btn sd-btn-warning">
