<x-app-layout>
    ...
    <style>
        ...
        .st-card.orange .st-card-hdr { background:linear-gradient(135deg,#fff7ed,#ffedd5); }
        .st-card.orange .st-card-ico { background:linear-gradient(135deg,#f97316,#ea580c); color:#fff; }
        ...
        .st-add-inp:focus { border-color:#f97316; background:#fff; box-shadow:0 0 0 3px rgba(249,115,22,0.1); }
        .st-add-btn {
            ...
            background:linear-gradient(135deg,#f97316,#ea580c); color:#fff; box-shadow:0 2px 8px rgba(234,88,12,0.25);
        }
        .st-add-btn:hover { transform:translateY(-1px); box-shadow:0 4px 12px rgba(234,88,12,0.35); }
    ...
    <div class="st-page">
        ...
        <nav class="st-nav">
            <a href="{{ route('minyak.produk.index') }}" class="st-back">
        ...
        <div class="st-card orange">
            <div class="st-card-hdr">
                <div class="st-card-hdr-left">
                    ...
                    <div>
                        ...
                        <div class="st-card-sub">Kategori / jenis minyak yang dijual</div>
                    </div>
                </div>
            ...
            <div class="st-card-body">
                {{-- Add Form --}}
                <form method="POST" action="{{ route('minyak.setting.jenis.store') }}" class="st-add-form">
                    @csrf
                    <div class="st-fg">
                        <label>Nama Jenis</label>
                        <input type="text" name="nama" class="st-add-inp @error('nama') is-invalid @enderror" placeholder="Contoh: Minyak Goreng Curah" required>
                    ...
                    <tbody>
                        ...
                        <tr id="jenis-row-{{ $item->id }}">
                            ...
                            <td>
                                ...
                                <form method="POST" action="{{ route('minyak.setting.jenis.update', $item) }}" class="st-edit-form" id="jenis-edit-{{ $item->id }}" style="display:none;">
                                    @csrf @method('PUT')
                            ...
                            <td>
                                <div class="st-actions">
                                    ...
                                    <form method="POST" action="{{ route('minyak.setting.jenis.destroy', $item) }}" onsubmit="return confirm('Hapus jenis \'{{ $item->nama }}\'?');" style="display:inline;">
                                        @csrf @method('DELETE')
        ...
        <div class="st-card green">
            <div class="st-card-hdr">
                <div class="st-card-hdr-left">
                    ...
                    <div>
                        ...
                        <div class="st-card-sub">Satuan pengukuran untuk produk minyak</div>
                    </div>
                </div>
            ...
            <div class="st-card-body">
                {{-- Add Form --}}
                <form method="POST" action="{{ route('minyak.setting.satuan.store') }}" class="st-add-form">
                    @csrf
                    <div class="st-fg">
                        <label>Nama Satuan</label>
                        <input type="text" name="nama" class="st-add-inp" placeholder="Contoh: Liter, Galon, Drum" required>
                        @error('nama')<div style="color:#dc2626;font-size:0.6875rem;">{{ $message }}</div>@enderror
                    </div>
                    <div class="st-fg st-fg-sm">
                        <label>Singkatan</label>
                        <input type="text" name="singkatan" class="st-add-inp" placeholder="L">
                    </div>
                    <button type="submit" class="st-add-btn">+ Tambah</button>
                </form>
                    ...
                    <tbody>
                        ...
                        <tr id="satuan-row-{{ $item->id }}">
                            ...
                            <td>
                                ...
                                <form method="POST" action="{{ route('minyak.setting.satuan.update', $item) }}" class="st-edit-form" id="satuan-edit-{{ $item->id }}" style="display:none;">
                                    @csrf @method('PUT')
                                    <input type="text" name="nama" value="{{ $item->nama }}" class="st-add-inp" required>
                                    <input type="hidden" name="status" value="{{ $item->status }}">
                                    <button type="submit" class="st-btn-sm st-btn-edit" title="Simpan">✓</button>
                            ...
                            <td>
                                <div class="st-actions">
                                    ...
                                    <form method="POST" action="{{ route('minyak.setting.satuan.destroy', $item) }}" onsubmit="return confirm('Hapus satuan \'{{ $item->nama }}\'?');" style="display:inline;">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="st-btn-sm st-btn-del">Hapus</button>
                                    </form>
                                </div>
