<x-app-layout>
    ...
    <style>
        ...
        /* header */
        .kn-header{background:linear-gradient(135deg,#eef2ff 0%,#e0e7ff 100%);border:1px solid #c7d2fe;border-radius:20px;padding:24px 28px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px}
        .kn-header-left{display:flex;align-items:center;gap:16px}
        .kn-header-icon{width:52px;height:52px;background:linear-gradient(135deg,#6366f1,#4f46e5);border-radius:16px;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 16px rgba(99,102,241,.35)}
        .kn-header-icon svg{width:26px;height:26px;color:#fff}
        .kn-header h1{font-size:1.375rem;font-weight:800;color:#1f2937;margin:0}
        .kn-header p{font-size:.8rem;color:#4338ca;margin:2px 0 0}
        .kn-date-badge{font-size:.75rem;font-weight:600;color:#4338ca;background:#fff;border:1px solid #c7d2fe;padding:6px 14px;border-radius:10px;display:flex;align-items:center;gap:6px}
        .kn-date-badge svg{width:14px;height:14px;color:#6366f1}
        .kn-hdr-btn{display:inline-flex;align-items:center;gap:.5rem;padding:.7rem 1.375rem;border-radius:12px;font-size:.8125rem;font-weight:600;text-decoration:none;transition:all .25s;border:none;cursor:pointer;background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;box-shadow:0 6px 20px rgba(79,70,229,.35)}
        .kn-hdr-btn:hover{transform:translateY(-2px);box-shadow:0 10px 32px rgba(79,70,229,.45)}
        ...
        .kn-kpi:hover{box-shadow:0 4px 14px rgba(99,102,241,.1)}
        .kn-kpi::before{content:'';position:absolute;left:0;top:0;bottom:0;width:4px}
        .kn-kpi.indigo::before{background:linear-gradient(180deg,#6366f1,#4f46e5)}
        ...
        .kn-kpi-icon.indigo{background:linear-gradient(135deg,#eef2ff,#e0e7ff);color:#4f46e5}
        ...
        .kn-date-lbl{font-size:.78rem;color:#4338ca;font-weight:600;white-space:nowrap}
        .kn-date-input{border:1.5px solid #c7d2fe;border-radius:12px;padding:9px 12px;font-size:.8125rem;background:#eef2ff;color:#4338ca;font-weight:500;outline:none;transition:border-color .2s}
        .kn-date-input:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.1)}
        .kn-filter select{border:1.5px solid #c7d2fe;border-radius:12px;padding:9px 14px;font-size:.8125rem;background:#eef2ff;color:#4338ca;font-weight:500;outline:none;min-width:150px;transition:border-color .2s;cursor:pointer}
        .kn-filter select:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.1)}
        .kn-btn-filter{background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;border:none;border-radius:12px;padding:9px 18px;font-size:.8125rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:7px;transition:opacity .2s}
        .kn-btn-filter:hover{opacity:.88}
        .kn-btn-filter svg{width:15px;height:15px}
        .kn-btn-reset{background:#eef2ff;color:#4338ca;border:1.5px solid #c7d2fe;border-radius:12px;padding:9px 16px;font-size:.8125rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:7px;text-decoration:none;transition:background .2s}
        .kn-btn-reset:hover{background:#e0e7ff}
        ...
        .kn-chart-title svg{width:18px;height:18px;color:#6366f1}
        ...
        .kn-bar-track{flex:1;height:28px;background:#eef2ff;border-radius:10px;overflow:hidden;position:relative}
        .kn-bar-fill{height:100%;background:linear-gradient(90deg,#6366f1,#4f46e5);border-radius:10px;display:flex;align-items:center;justify-content:flex-end;padding-right:10px;min-width:32px;transition:width .4s}
        ...
        .kn-tbl thead th{background:linear-gradient(180deg,#eef2ff,#f5f3ff);border-bottom:2px solid #c7d2fe;padding:13px 16px;font-size:.7rem;font-weight:700;color:#4338ca;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap}
        ...
        .kn-tbl tbody tr:hover{background:#eef2ff}
        ...
        .kn-sales-av{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#6366f1,#4f46e5);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:.72rem;flex-shrink:0}
        ...
        .kn-status.berlangsung{background:#eef2ff;color:#4338ca}
        .kn-status.berlangsung .kn-status-dot{background:#6366f1;animation:kn-pulse 1.8s infinite}
        ...
        /* action */
        .kn-act{display:inline-flex;align-items:center;gap:5px;padding:6px 12px;border-radius:10px;font-size:.72rem;font-weight:600;text-decoration:none;transition:background .2s;border:none;cursor:pointer;background:#eef2ff;color:#4f46e5}
        .kn-act:hover{background:#e0e7ff}
        .kn-act svg{width:13px;height:13px}

        /* pagination */
        .kn-pagination{padding:16px 20px;border-top:1px solid #e2e8f0;background:linear-gradient(180deg,#eef2ff,#f5f3ff)}

        /* empty */
        .kn-empty{text-align:center;padding:60px 24px}
        .kn-empty-icon{width:80px;height:80px;margin:0 auto 18px;background:linear-gradient(135deg,#eef2ff,#e0e7ff);border-radius:24px;display:flex;align-items:center;justify-content:center}
        .kn-empty-icon svg{width:38px;height:38px;color:#6366f1}
    ...
    <div class="kn-wrap" style="padding:24px">
        ...
        <div class="kn-header">
            <div class="kn-header-left">
                ...
                <div>
                    ...
                    <p>Tracking kunjungan sales minyak</p>
                </div>
            </div>
            <div style="display:flex;align-items:center;gap:12px;">
                ...
                @if($isSalesRole)
                <a href="{{ route('minyak.kunjungan.checkin') }}" class="kn-hdr-btn">
        ...
        <div class="kn-kpi-grid">
            <div class="kn-kpi indigo">
                <div class="kn-kpi-icon indigo">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                </div>
                <div>
        ...
        <div class="kn-filter">
            <form method="GET">
                ...
                <a href="{{ route('minyak.kunjungan.index') }}" class="kn-btn-reset">
                ...
                <table class="kn-tbl">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align:center">
                                <a href="{{ route('minyak.kunjungan.show', $k) }}" class="kn-act">
