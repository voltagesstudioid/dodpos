<x-app-layout>
    ...
    <style>
        ...
        .db-kpi-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem}
        .db-kpi-lbl{font-size:.6875rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8}
        .db-kpi-ico{width:40px;height:40px;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:1.125rem;flex-shrink:0}
        ...
        .db-kpi-val{font-size:1.5rem;font-weight:800;color:#0f172a;letter-spacing:-.03em;line-height:1.2}
        .db-kpi-val .u{font-size:.75rem;font-weight:600;color:#94a3b8;margin-left:3px}
        ...
        .db-feat-top{display:flex;align-items:center;gap:.75rem;margin-bottom:.75rem}
        ...
        .db-feat-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:.5rem;margin-bottom:.75rem}
        ...
        /* Panels */
        .db-panel{background:#fff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;transition:box-shadow .25s;margin-bottom:0}
        ...
        /* Grid layouts */
        .db-grid-2-1{display:grid;grid-template-columns:2fr 1fr;gap:1rem;margin-bottom:1.25rem}
        ...
        .db-nav-ico.rose{background:linear-gradient(135deg,#fff1f2,#ffe4e6)}
        ...
        /* Hutang alert box */
        .db-alert-box{padding:.875rem 1rem;background:linear-gradient(135deg,#fef2f2,#fff1f2);border-radius:12px;border:1px solid #fecaca;margin-bottom:.875rem}
        .db-alert-lbl{font-size:.625rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin-bottom:.375rem}
        .db-alert-val{font-size:1.25rem;font-weight:800;color:#dc2626;letter-spacing:-.03em}

        /* Oversized stat */
        .db-overdue-badge{display:inline-flex;align-items:center;gap:.25rem;padding:.125rem .625rem;border-radius:99px;font-size:.6875rem;font-weight:700}

        /* Responsive */
        @media(max-width:1280px){.db-feat-stats{grid-template-columns:repeat(2,1fr)}}
        @media(max-width:1024px){.db-kpis{grid-template-columns:repeat(2,1fr)}.db-feat{grid-template-columns:1fr}.db-nav{grid-template-columns:repeat(2,1fr)}.db-grid-2-1{grid-template-columns:1fr}.db-grid-3{grid-template-columns:1fr}}
        @media(max-width:768px){.db-hero{padding:1.5rem}.db-hero-title{font-size:1.375rem}.db-hero-sub{font-size:.75rem}.db-hero-btn{padding:.5rem 1rem;font-size:.75rem}}
        @media(max-width:640px){.db-kpis{grid-template-columns:1fr}.db-nav{grid-template-columns:1fr}.db-hero{padding:1.25rem}.db-hero-title{font-size:1.125rem}.db-hero-top{flex-direction:column;align-items:stretch}.db-hero-actions{flex-direction:column}.db-hero-btn{justify-content:center}.db-kpi-val{font-size:1.25rem}.db-feat-stats{grid-template-columns:1fr}.db-leader-item{flex-wrap:wrap;gap:.5rem}.db-leader-amt{width:100%;text-align:right}.db-pending-item{flex-direction:column;align-items:flex-start;gap:.5rem}}
        @media(max-width:480px){.db{padding:1rem .75rem 2rem}.db-hero{border-radius:14px;padding:1rem}.db-hero-title{font-size:1rem}.db-kpi{padding:1rem 1.125rem}.db-panel-hdr{padding:.875rem 1rem}.db-panel-body{padding:.875rem 1rem}}
    </style>
    @endpush
    ...
    <div class="db">
        ...
        <div class="db-hero">
            <div class="db-hero-top">
                <div>
                    ...
                    <p class="db-hero-sub">Ringkasan penjualan dan operasional</p>
        ...
        <div class="db-kpis">
            ...
            <div class="db-kpi blue">
                <div class="db-kpi-top"><span class="db-kpi-lbl">Hutang Aktif</span><div class="db-kpi-ico amber">💳</div></div>
                <div class="db-kpi-val">Rp {{ number_format($totalHutang, 0, ',', '.') }}</div>
                <div class="db-kpi-foot">Overdue: <span style="color:{{ $hutangOverdue > 0 ? '#dc2626' : '#059669' }};font-weight:700">{{ $hutangOverdue }}</span></div>
            </div>
        </div>
            ...
            <div class="db-feat-card">
                ...
                <div class="db-feat-body">
                    ...
                    <div class="db-feat-stats">
                        ...
                        <div class="db-feat-stat"><div class="db-feat-stat-lbl">Setoran Pending</div><div class="db-feat-stat-val" style="color:#d97706">{{ $setoranPending->count() }}</div></div>
                    </div>
        ...
        <div class="db-grid-2-1">
            ...
            <div class="db-panel">
                ...
                <div class="db-panel-body">
                    ...
                    <div class="db-stat-row"><div class="db-stat-left"><div class="db-stat-dot amber"></div><span class="db-stat-name">Pelanggan Aktif</span></div><span class="db-stat-val">{{ $master['total_pelanggan'] }}</span></div>
                </div>
            </div>
        </div>

        {{-- Middle Row: Hutang Status + Setoran Pending + Master Data --}}
        <div class="db-grid-3">
            <div class="db-panel">
                <div class="db-panel-hdr">
                ...
                <div class="db-panel-body">
                    ...
                    <div class="db-stat-row"><div class="db-stat-left"><div class="db-stat-dot green"></div><span class="db-stat-name">Pelanggan Aktif</span></div><span class="db-stat-val">{{ $master['total_pelanggan'] }}</span></div>
                    <div class="db-stat-row"><div class="db-stat-left"><div class="db-stat-dot purple"></div><span class="db-stat-name">Produk Aktif</span></div><span class="db-stat-val">{{ $master['total_produk'] }}</span></div>
                </div>
            </div>
            ...
            <div class="db-panel">
                ...
                <div class="db-panel-body">
                    <div class="db-alert-box">
                        <div class="db-alert-lbl">Total Hutang Aktif</div>
                        <div class="db-alert-val">Rp {{ number_format($totalHutang, 0, ',', '.') }}</div>
                    </div>
                    <div class="db-stat-row"><div class="db-stat-left"><div class="db-stat-dot red"></div><span class="db-stat-name">Overdue</span></div><span class="db-overdue-badge" style="{{ $hutangOverdue > 0 ? 'background:#fef2f2;color:#dc2626;border:1px solid #fecaca' : 'background:#ecfdf5;color:#059669;border:1px solid #a7f3d0' }}">{{ $hutangOverdue > 0 ? '⚠️' : '✅' }} {{ $hutangOverdue }}</span></div>
                    <div class="db-stat-row"><div class="db-stat-left"><div class="db-stat-dot amber"></div><span class="db-stat-name">Setoran Pending</span></div><span style="font-weight:700;color:{{ $setoranPending->count() > 0 ? '#d97706' : '#94a3b8' }}">{{ $setoranPending->count() }}</span></div>
                </div>
            </div>
        ...
        {{-- Top Sales --}}
        <div class="db-panel" style="margin-bottom:1.25rem">
            <div class="db-panel-hdr">
        ...
        <div class="db-nav">
            ...
            <a href="{{ route('minyak.sales.index') }}" class="db-nav-card"><div class="db-nav-ico violet">👥</div><div><div class="db-nav-title">Data Sales</div><div class="db-nav-sub">Kelola sales</div></div></a>
            <a href="{{ route('minyak.hutang.piutang') }}" class="db-nav-card"><div class="db-nav-ico rose">💳</div><div><div class="db-nav-title">Piutang</div><div class="db-nav-sub">Hutang pelanggan</div></div></a>
        </div>
