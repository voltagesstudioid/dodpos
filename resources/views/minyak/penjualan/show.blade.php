<x-app-layout>
    ...
    <style>
        ...
        .sd-hdr { background:linear-gradient(135deg,#065f46 0%,#059669 50%,#10b981 100%); border-radius:20px; padding:1.5rem 1.75rem; margin-bottom:1.5rem; box-shadow:0 12px 40px rgba(5,150,105,0.2); position:relative; overflow:hidden; }
        ...
        .sd-total-box { background:linear-gradient(135deg,#ecfdf5,#d1fae5); border:1px solid #a7f3d0; border-radius:12px; padding:1rem 1.25rem; margin-top:0.75rem; display:flex; justify-content:space-between; align-items:center; }
        .sd-total-lbl { font-size:0.875rem; font-weight:700; color:#065f46; }
        .sd-total-val { font-size:1.25rem; font-weight:800; color:#065f46; }
        ...
        .sd-btn-verify { background:linear-gradient(135deg,#10b981,#059669); color:#fff; box-shadow:0 6px 20px rgba(5,150,105,0.25); flex:1; }
    ...
    <div class="sd-page">
        <a href="{{ route('minyak.penjualan.index') }}" class="sd-back">
        ...
        <div class="sd-card">
            ...
            <div class="sd-card-body">
                ...
                <div class="sd-row"><span class="sd-key">No. Bukti Transfer</span><span class="sd-val" style="font-family:'JetBrains Mono',monospace;font-weight:600;color:#0284c7;">{{ $penjualan->no_bukti_transfer }}</span></div>
                @endif
                @if(!$isSalesRole && $penjualan->tipe_bayar === 'hutang' && $penjualan->bayar > 0)
                <div class="sd-row"><span class="sd-key">Dibayar (DP)</span><span class="sd-val">Rp {{ number_format($penjualan->bayar, 0, ',', '.') }}</span></div>
                @endif
                @if(!$isSalesRole && $penjualan->hutang > 0)
        ...
        <div class="sd-card">
            ...
            <div class="sd-card-body">
                <div class="sd-row">
                    ...
                    <a href="https://maps.google.com/?q={{ $penjualan->latitude }},{{ $penjualan->longitude }}" target="_blank" class="sd-val" style="color:#4f46e5;">
        ...
        <div class="sd-actions">
            <a href="{{ route('minyak.penjualan.index') }}" class="sd-btn sd-btn-back">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                Kembali
            </a>
            @if(!$isSalesRole && $penjualan->status === 'pending' && Route::has('minyak.penjualan.edit'))
            <a href="{{ route('minyak.penjualan.edit', $penjualan) }}" class="sd-btn sd-btn-edit">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                Edit
            </a>
            @endif
            @if(Route::has('minyak.penjualan.print'))
            <a href="{{ route('minyak.penjualan.print', $penjualan) }}" target="_blank" class="sd-btn sd-btn-print">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                Cetak Struk
            </a>
            @endif
            @if(!$isSalesRole && $penjualan->status !== 'terverifikasi' && Route::has('minyak.penjualan.verify'))
            <form method="POST" action="{{ route('minyak.penjualan.verify', $penjualan) }}" style="flex:1;">
                @csrf
            ...
            @endif
            @if(!$isSalesRole && $penjualan->status === 'pending' && Route::has('minyak.penjualan.destroy'))
            <form method="POST" action="{{ route('minyak.penjualan.destroy', $penjualan) }}" onsubmit="return confirm('Yakin batalkan transaksi ini?')">
                @csrf
                @method('DELETE')
