<x-app-layout>
    ...
    <style>
        ...
        .pj-hdr-left {}
        .pj-hdr-tag { display:inline-block; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:#10b981; background:#ecfdf5; padding:3px 10px; border-radius:20px; margin-bottom:6px; }
        ...
        .pj-sec {
            ...
            margin-bottom:14px; box-shadow:0 1px 2px rgba(0,0,0,.03), 0 4px 12px rgba(0,0,0,.02);
            transition:box-shadow .2s;
        }
        .pj-sec:hover { box-shadow:0 1px 2px rgba(0,0,0,.04), 0 8px 24px rgba(0,0,0,.04); }
        .pj-sec-hdr {
            padding:.875rem 1.25rem; display:flex; align-items:center; gap:10px;
        ...
        .pj-grid-3 { display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; }
        ...
        .pj-inp {
            ...
            background:#fafbfc; font-size:14px; color:#0f172a; outline:none; transition:all .2s;
            font-family:inherit;
        }
        ...
        /* Select */
        .pj-sel {
            width:100%; padding:10px 36px 10px 14px; border-radius:10px; border:1.5px solid #e2e8f0;
            background:#fafbfc; font-size:14px; color:#0f172a; outline:none; transition:all .2s;
        ...
        .pj-radio-face {
            ...
            padding:12px 8px; border-radius:10px; border:2px solid #e2e8f0;
            background:#fafbfc; transition:all .2s; text-align:center;
        }
        ...
        .pj-radio-pill input:checked + .pj-radio-face {
            border-color:#10b981; background:linear-gradient(180deg,#ecfdf5,#f0fdf4);
        ...
        /* Photo upload */
        .pj-photo-box {
            position:relative; width:100%; aspect-ratio:16/9; background:linear-gradient(135deg,#f8fafc,#f1f5f9);
            border:2px dashed #d1d5db; border-radius:12px; overflow:hidden;
            display:flex; align-items:center; justify-content:center; transition:border-color .2s;
        }
        .pj-photo-box:hover { border-color:#a7f3d0; }
        ...
        .pj-photo-empty-ico {
            width:44px; height:44px; background:#e2e8f0; border-radius:50%;
        ...
        .pj-cam-btn {
            ...
            border:1.5px solid #a7f3d0; cursor:pointer; transition:all .2s; font-family:inherit;
            background:#f0fdf4; color:#065f46; margin-top:10px;
        }
        .pj-cam-btn:hover { background:#ecfdf5; border-color:#6ee7b7; }
        .pj-cap-btn {
            display:none; width:100%; padding:10px; border-radius:10px; font-size:13px; font-weight:700;
            border:1.5px solid #86efac; cursor:pointer; font-family:inherit;
            background:#dcfce7; color:#166534; margin-top:8px; text-align:center;
        }

        /* Sticky submit bar */
        .pj-submit-bar {
            position:sticky; bottom:0; background:linear-gradient(to top,#fff 60%,transparent);
            padding:16px 0 4px; display:flex; gap:10px; margin-top:8px; z-index:10;
        }
        .pj-btn-cancel {
            display:inline-flex; align-items:center; justify-content:center; gap:6px;
            padding:12px 20px; border-radius:12px; font-size:14px; font-weight:600;
            border:1.5px solid #e2e8f0; cursor:pointer; transition:all .2s; font-family:inherit;
            background:#fff; color:#64748b; text-decoration:none; white-space:nowrap;
        }
        ...
        .pj-btn-submit:hover { transform:translateY(-1px); box-shadow:0 8px 28px rgba(5,150,105,.35); }
        .pj-btn-submit:active { transform:translateY(0); }
        .pj-btn-submit:disabled { opacity:0.5; cursor:not-allowed; transform:none; box-shadow:none; background:#94a3b8; }
        ...
        @media(max-width:640px) {
            ...
            .pj-submit-bar { flex-direction:column; }
        }
    </style>
    @endpush

    <div class="pj-page">
        <a href="{{ route('minyak.penjualan.index') }}" class="pj-back">
            <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Kembali ke Penjualan
        </a>

        <div class="pj-hdr">
            <div class="pj-hdr-left">
                <div class="pj-hdr-tag">Transaksi Baru</div>
                <div class="pj-hdr-title">Tambah Penjualan</div>
                <div class="pj-hdr-sub">Catat transaksi penjualan minyak baru</div>
            </div>
        </div>

        <form method="POST" action="{{ route('minyak.penjualan.store') }}" enctype="multipart/form-data" id="form-penjualan">
            @csrf
                ...
                <div class="pj-sec-body">
                    ...
                    <div class="pj-sum" id="summary-box" style="display:none;">
                        <div class="pj-sum-row">
                            ...
                            <span class="pj-sum-v" id="sum-qty">0</span>
                        </div>
                        <div class="pj-sum-row">
                            <span class="pj-sum-k">Harga Satuan</span>
                            <span class="pj-sum-v" id="sum-price">Rp 0</span>
                        </div>
                        <div class="pj-sum-row total">
                            <span class="pj-sum-k" style="font-weight:700;">Total</span>
                            <span class="pj-sum-v hi" id="sum-total">Rp 0</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="pj-sec">
                ...
                <div class="pj-sec-body">
                    <div class="pj-fld">
                        ...
                        <div class="pj-radio-grp">
                            <label class="pj-radio-pill">
                                <input type="radio" name="tipe_bayar" value="tunai" {{ old('tipe_bayar', 'tunai') === 'tunai' ? 'checked' : '' }}>
                                <div class="pj-radio-face">
                                    <span class="ico">💵</span>
                                    <span class="lbl">Tunai</span>
                                </div>
                            </label>
                            <label class="pj-radio-pill">
                                <input type="radio" name="tipe_bayar" value="hutang" {{ old('tipe_bayar') === 'hutang' ? 'checked' : '' }}>
                                <div class="pj-radio-face">
                                    <span class="ico">📄</span>
                                    <span class="lbl">Hutang</span>
                                </div>
                            </label>
                            <label class="pj-radio-pill">
                                <input type="radio" name="tipe_bayar" value="transfer" {{ old('tipe_bayar') === 'transfer' ? 'checked' : '' }}>
                                <div class="pj-radio-face">
                                    <span class="ico">🏦</span>
                    ...
                    <div class="pj-fld" id="fld-bukti-transfer" style="display:none;">
                        <label class="pj-lbl">
                        ...
                        <div class="pj-photo-box" id="transfer-preview-box" style="aspect-ratio:4/3;border-color:#a7f3d0;">
                            <img id="transfer-preview" src="" alt="" style="display:none;">
                            <div class="pj-photo-empty" id="transfer-placeholder">
                                <div class="pj-photo-empty-ico" style="background:#d1fae5;">
                                    <svg width="22" height="22" fill="none" stroke="#059669" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                </div>
                    ...
                    {{-- Credit Limit Info --}}
                    <div id="credit-limit-box" style="display:none; margin-top:0.75rem; padding:0.75rem 1rem; border-radius:10px; font-size:0.75rem; line-height:1.6;">
                        <div id="credit-limit-content"></div>
                    </div>
                ...
                <div class="pj-sec-body">
                    ...
                    <div class="pj-photo-box" id="nota-preview-box">
                        ...
                        <div class="pj-photo-empty" id="nota-placeholder">
                            <div class="pj-photo-empty-ico">
                                <svg width="22" height="22" fill="none" stroke="#94a3b8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            </div>
                    ...
                    @error('foto_nota')<div class="pj-err">{{ $message }}</div>@enderror
                </div>
            </div>

            {{-- Sticky Submit --}}
            <div class="pj-submit-bar">
                <a href="{{ route('minyak.penjualan.index') }}" class="pj-btn-cancel">
                ...
                <button type="submit" id="btn-submit" class="pj-btn-submit">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="tipe_bayar"]').forEach(function(r) {
            r.addEventListener('change', function() {
                ...
                document.getElementById('fld-bukti-transfer').style.display = this.value === 'transfer' ? 'block' : 'none';
                checkCreditLimit();
            });
        });

        // Credit limit check
        var selPelanggan = document.getElementById('sel-pelanggan');
        var creditBox = document.getElementById('credit-limit-box');
        var creditContent = document.getElementById('credit-limit-content');
        var submitBtn = document.getElementById('btn-submit');

        function checkCreditLimit() {
            var tipeBayar = document.querySelector('input[name="tipe_bayar"]:checked');
            if (!tipeBayar || tipeBayar.value !== 'hutang') {
            ...
            var totalHutang = parseFloat(opt.dataset.total) || 0;
            var sisaLimit = Math.max(0, limit - totalHutang);
            ...
            if (limit <= 0) {
                ...
                // Exceeds limit!
                html = '<div style="color:#991b1b; font-weight:700;">⚠️ MELEBIHI LIMIT KREDIT!</div>';
                html += '<div style="color:#dc2626; margin-top:4px;">Hutang baru: <strong>' + fmt(hutangBaru) + '</strong></div>';
                html += '<div style="color:#dc2626;">Sisa limit: <strong>' + fmt(sisaLimit) + '</strong></div>';
                html += '<div style="color:#dc2626;">Kelebihan: <strong>' + fmt(hutangBaru - sisaLimit) + '</strong></div>';
                html += '<div style="color:#7f1d1d; margin-top:6px; font-size:0.6875rem;">Transaksi tidak dapat diproses. Kurangi jumlah atau tambah DP.</div>';
                creditBox.style.background = '#fef2f2';
                creditBox.style.border = '1.5px solid #fecaca';
                ...
                creditBox.style.border = '1.5px solid #a7f3d0';
                if (submitBtn) submitBtn.disabled = false;
            }
        ...
        selPelanggan.addEventListener('change', checkCreditLimit);
        document.getElementById('inp-jumlah').addEventListener('input', checkCreditLimit);
        document.getElementById('inp-harga').addEventListener('input', checkCreditLimit);
        document.getElementById('inp-bayar').addEventListener('input', checkCreditLimit);
        ...
        if (checked) {
            ...
            document.getElementById('fld-bukti-transfer').style.display = checked.value === 'transfer' ? 'block' : 'none';
        }
        updateSummary();
        ...
        (function() {
            ...
            var inp = document.getElementById('nota-input');
            var preview = document.getElementById('nota-preview');
            var ph = document.getElementById('nota-placeholder');
            var b64 = document.getElementById('nota-base64');
            var btnCam = document.getElementById('nota-btn-camera');
            var btnCap = document.getElementById('nota-btn-capture');
            var vid = document.getElementById('nota-webcam-video');
            var cvs = document.getElementById('nota-webcam-canvas');
            var stream = null;

            function show(src) {
                preview.src = src; preview.style.display = 'block';
                ph.style.display = 'none'; vid.style.display = 'none';
                btnCap.style.display = 'none'; btnCam.innerHTML = 'Ulangi Foto';
            }
            ...
            async function start() {
                try {
                    ...
                    preview.style.display='none'; ph.style.display='none';
                    btnCam.style.display='none'; btnCap.style.display='block';
            ...
            btnCap.addEventListener('click', function(){
                ...
                var d=cvs.toDataURL('image/jpeg',.8); b64.value=d; stop(); show(d);
            });
            inp.addEventListener('change', function(){ var f=this.files&&this.files[0]; if(!f)return; show(URL.createObjectURL(f)); });
        })();
    });
