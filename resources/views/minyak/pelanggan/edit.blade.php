    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <form method="POST" action="{{ route('minyak.pelanggan.update', $pelanggan) }}" class="p-6">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-4 p-3 bg-gray-50 rounded">
                        <span class="text-sm text-gray-500">Kode Pelanggan:</span>
                        <span class="font-bold">{{ $pelanggan->kode_pelanggan }}</span>
                    </div>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Nama Toko</label>
                            <input type="text" name="nama_toko" value="{{ old('nama_toko', $pelanggan->nama_toko) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_pemilik" value="{{ old('nama_pemilik', $pelanggan->nama_pemilik) }}" required
                                class="mt-1 block w-full border rounded-lg px-4 py-2">
                        ...
                        <div>
                            ...
                            <textarea name="alamat" rows="3" class="mt-1 block w-full border rounded-lg px-4 py-2">{{ old('alamat', $pelanggan->alamat) }}</textarea>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="kecamatan" value="{{ old('kecamatan', $pelanggan->kecamatan) }}"
                                    class="mt-1 block w-full border rounded-lg px-4 py-2">
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <select name="tipe" required class="mt-1 block w-full border rounded-lg px-4 py-2">
                                <option value="eceran" {{ old('tipe', $pelanggan->tipe) == 'eceran' ? 'selected' : '' }}>Eceran</option>
                                <option value="grosir" {{ old('tipe', $pelanggan->tipe) == 'grosir' ? 'selected' : '' }}>Grosir</option>
                                <option value="agen" {{ old('tipe', $pelanggan->tipe) == 'agen' ? 'selected' : '' }}>Agen</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="limit_hutang" value="{{ old('limit_hutang', $pelanggan->limit_hutang) }}"
                                class="mt-1 block w-full border rounded-lg px-4 py-2">
