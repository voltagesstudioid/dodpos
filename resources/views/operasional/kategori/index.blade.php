<x-app-layout>
        ...
        <div class="tr-page">
            ...
            <div class="tr-header">
                <div class="tr-header-text">
                    <div class="tr-eyebrow">Pengaturan Pengeluaran</div>
                    <h1 class="tr-title">
                    ...
                    <p class="tr-subtitle">Kelola klasifikasi pengeluaran operasional toko Anda untuk laporan yang lebih detail.</p>
                </div>

                <div class="tr-header-actions">
                    @can('create_kategori_operasional')
                        <a href="{{ route('operasional.kategori.create') }}" class="tr-btn tr-btn-primary">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                            Tambah Kategori
                        </a>
                    @else
                        <button class="tr-btn tr-btn-disabled" title="Izin diperlukan: create_kategori_operasional" disabled>
            ...
            <div class="tr-stats-row">
                <div class="tr-stat-card">
                    ...
                    <div>
                        <div class="tr-stat-value">{{ $totalCategories }}</div>
                        <div class="tr-stat-label">Total Kategori</div>
                    </div>
                </div>
                <div class="tr-stat-card">
                    ...
                    <div>
                        <div class="tr-stat-value">{{ $categoriesWithExpenses }}</div>
                ...
                <a href="{{ route('operasional.kategori.export') }}" class="tr-stat-card tr-stat-clickable">
            ...
            <div class="tr-filter-bar">
                <form method="GET" action="{{ route('operasional.kategori.index') }}" class="tr-filter-form">
                    <div class="tr-search-box">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                        <input type="text" name="search" class="tr-search-input" placeholder="Cari kategori..." value="{{ request('search') }}">
                    </div>
                    <button type="submit" class="tr-btn tr-btn-dark">Cari</button>
                    @if(request('search'))
                        <a href="{{ route('operasional.kategori.index') }}" class="tr-btn tr-btn-outline">Reset</a>
                    @endif
                </form>
            </div>
                        ...
                        <tbody>
                            @forelse ($categories as $index => $kategori)
                                <tr>
                                    ...
                                    <td>
                                        <div class="tr-font-bold tr-text-main">{{ $kategori->name }}</div>
                                    </td>
                                    <td>
                                        <div class="tr-text-muted tr-desc-truncate">{{ $kategori->description ?: '— Tidak ada deskripsi' }}</div>
                                    </td>
                                    <td class="r">
                                        <div class="tr-actions-group">
                                            @can('edit_kategori_operasional')
                                                <a href="{{ route('operasional.kategori.edit', $kategori->id) }}" class="tr-action-btn edit" title="Edit">
                                            ...
                                            @can('delete_kategori_operasional')
                                                <form action="{{ route('operasional.kategori.destroy', $kategori->id) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?');" class="tr-inline">
                                                    @csrf @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="4">
                                        <div class="tr-empty-state">
                                            ...
                                            <p>Data kategori pengeluaran Anda akan muncul di sini.</p>
                                            @can('create_kategori_operasional')
                                                <a href="{{ route('operasional.kategori.create') }}" class="tr-link">Buat Kategori Pertama &rarr;</a>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
    ...
    <style>
        ...
        .tr-desc-truncate { max-width: 400px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        @media (max-width: 640px) {
            ...
            .tr-desc-truncate { max-width: 200px; }
        }
    </style>
    @endpush
