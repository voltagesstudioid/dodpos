@extends('layouts.app', ['title' => 'Detail Setoran - Pasgar'])
...
<style>
    ...
    .sd-method.qris { background:#ede9fe; color:#6d28d9; }
...
<div class="sd-page">
    <nav class="sd-nav">
        <a href="{{ route('pasgar.setoran.index') }}" class="sd-back">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            Daftar Setoran
        </a>
        <span class="sd-sep">/</span>
        <span class="sd-crumb">{{ $setoran->nomor_setoran }}</span>
    </nav>
    ...
    <div class="sd-hdr {{ $setoran->status }}">
        <div class="sd-hdr-left">
            ...
            <div>
                <div class="sd-hdr-title">{{ $setoran->nomor_setoran }}</div>
        ...
        <div style="display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap;">
            ...
            <div class="sd-hdr-actions">
                ...
                <a href="{{ route('pasgar.setoran.edit', $setoran->id) }}" class="sd-btn sd-btn-edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit</a>
                <form action="{{ route('pasgar.setoran.destroy', $setoran->id) }}" method="POST" onsubmit="return confirm('Yakin hapus setoran ini?')" style="display:inline;">@csrf @method('DELETE')
    ...
    <div class="sd-card blue">
        ...
        <div class="sd-card-body">
            <div class="cf-step">
                ...
                <div class="cf-step-label"><strong>Penjualan Tunai</strong> — {{ $penjualans->where('metode_bayar','tunai')->count() }} transaksi<div class="cf-step-sub">Uang tunai diterima langsung dari pelanggan</div></div>
                <div class="cf-step-value green">Rp {{ number_format($tunaiFromCash, 0, ',', '.') }}</div>
            </div>
            <div class="cf-step" style="padding-top:0.5rem;">
                <div class="cf-step-num total">2</div>
                <div class="cf-step-label"><strong>Seharusnya Disetor</strong><div class="cf-step-sub">Total uang tunai yang wajib disetor</div></div>
                <div class="cf-step-value amber" style="font-size:0.95rem;">Rp {{ number_format($seharusnyaSetor, 0, ',', '.') }}</div>
            </div>
            <hr class="cf-divider">
            <div class="cf-step">
                <div class="cf-step-num result">3</div>
            ...
            @endphp
            <div class="cf-step">
                <div class="cf-step-num" style="background:{{ $selisihColor }}20; color:{{ $selisihColor }};">4</div>
                <div class="cf-step-label"><strong>Selisih</strong> = (3) - (2)<div class="cf-step-sub">{{ $setoran->selisih == 0 ? 'Setoran pas, tidak ada selisih' : ($setoran->selisih > 0 ? 'Sales setor LEBIH dari seharusnya' : 'Sales setor KURANG dari seharusnya') }}</div></div>
            ...
            <div class="sd-selisih {{ $selisihType }}">
                <div class="sd-selisih-label" style="color:{{ $selisihColor }};">Status: {{ strtoupper($setoran->selisih_label) }}</div>
            ...
            {{-- Info: Transfer/QRIS tidak masuk setoran tunai --}}
            ...
            <div style="margin-top:0.75rem;padding:0.55rem 0.85rem;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;font-size:0.75rem;color:#1d4ed8;display:flex;align-items:flex-start;gap:0.4rem;">
                ...
                <div>Transfer/QRIS sebesar <strong>Rp {{ number_format($summary['total_transfer'], 0, ',', '.') }}</strong> <u>tidak termasuk</u> dalam setoran tunai (sudah masuk rekening bank).</div>
            </div>
            @endif
        ...
        <div class="sd-card-body" style="padding:0;">
            ...
            <table class="sd-tbl">
                ...
                <tbody>
                    ...
                    @php
                        $kasMasuk = 0;
                        if ($p->metode_bayar === 'tunai') { $kasMasuk = (float) $p->total; }
                    @endphp
                    <tr>
                        <td class="sd-mono">{{ $i + 1 }}</td>
                        <td><div class="sd-mono sd-bold">{{ $p->nomor_transaksi }}</div><div style="font-size:0.68rem;color:#94a3b8;">{{ $p->tanggal->format('d/m/Y H:i') }}</div></td>
                        <td class="sd-bold">{{ $p->nama_pelanggan ?? ($p->pelanggan->nama ?? '-') }}</td>
                        <td class="sd-center"><span class="sd-method {{ $p->metode_bayar }}">{{ strtoupper($p->metode_bayar) }}</span></td>
                ...
                <tfoot>
                    <tr>
                        <td colspan="4" class="sd-bold">Total</td>
                        <td class="sd-right sd-mono sd-bold">Rp {{ number_format($penjualans->sum('total'), 0, ',', '.') }}</td>
                        <td class="sd-right sd-mono sd-bold" style="color:#059669;">Rp {{ number_format($tunaiFromCash, 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
            </div>
...
<table class="sd-info">
    ...
    <div class="sd-verify">
        ...
        <form action="{{ route('pasgar.setoran.verify', $setoran->id) }}" method="POST">
            @csrf
