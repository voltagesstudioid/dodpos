...
<head>
    ...
    <title>Laporan Penjualan Pasgar</title>
...
<body>
    ...
    <div class="page no-print">
        <form method="GET" action="{{ route('pasgar.laporan.penjualan') }}" class="filter-bar">
            ...
            <a href="{{ route('pasgar.laporan.penjualan', ['date_from' => $dateFrom, 'date_to' => $dateTo, 'sales_id' => $salesId, 'print' => 1]) }}" target="_blank" class="btn btn-print">🖨️ Cetak</a>
    ...
    <div class="page">
        ...
        <div class="hdr">
            <div class="hdr-left">
                <h1>Laporan Penjualan Pasgar</h1>
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="summary">
                    <div class="s-card">
                        ...
                        <div class="s-card-sub">{{ $summary['totalTransaksi'] }} transaksi</div>
                    </div>
                    <div class="s-card blue">
                        <div class="s-card-label">Tunai</div>
                        <div class="s-card-value">Rp {{ number_format($summary['totalTunai'], 0, ',', '.') }}</div>
                        <div class="s-card-sub">pembayaran tunai</div>
                    </div>
                    <div class="s-card purple">
                        <div class="s-card-label">Transfer/QRIS</div>
                        <div class="s-card-value">Rp {{ number_format($summary['totalTransfer'], 0, ',', '.') }}</div>
                        <div class="s-card-sub">transfer & QRIS</div>
                    </div>
                    <div class="s-card red">
                        <div class="s-card-label">Rata-rata / Transaksi</div>
                        <div class="s-card-value">Rp {{ $summary['totalTransaksi'] > 0 ? number_format($summary['totalPenjualan'] / $summary['totalTransaksi'], 0, ',', '.') : '0' }}</div>
                        <div class="s-card-sub">rata-rata nilai transaksi</div>
                    </div>
                </div>
            </div>
            ...
            {{-- Per-sales breakdown --}}
            <div class="section">
                <div class="section-title">Penjualan Per Sales</div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right mono">Rp {{ number_format($row->total_penjualan, 0, ',', '.') }}</td>
                            <td class="text-right mono">Rp {{ number_format($row->total_tunai, 0, ',', '.') }}</td>
                            <td class="text-right mono">Rp {{ number_format($row->total_transfer, 0, ',', '.') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-right mono">Rp {{ number_format($summary['totalPenjualan'], 0, ',', '.') }}</td>
                            <td class="text-right mono">Rp {{ number_format($summary['totalTunai'], 0, ',', '.') }}</td>
                            <td class="text-right mono">Rp {{ number_format($summary['totalTransfer'], 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
                @else
                <div class="empty">Tidak ada data penjualan</div>
                @endif
            </div>
            ...
            <div class="section">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $i + 1 }}</td>
                            <td class="mono text-bold">{{ $d->nomor_transaksi }}</td>
                            <td>{{ \Carbon\Carbon::parse($d->tanggal)->format('d/m/Y') }}</td>
                            <td>{{ $d->sales?->nama ?? '-' }}</td>
                            <td>{{ $d->nama_pelanggan ?? 'Non-pelanggan' }}</td>
                            <td>
                            ...
                            <td class="text-right mono text-bold">Rp {{ number_format($d->total, 0, ',', '.') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="6">Total ({{ $details->count() }} transaksi)</td>
                            <td class="text-right mono">Rp {{ number_format($details->sum('total'), 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="footer">
            Laporan Penjualan Pasgar &middot; DOD POS &middot; {{ now()->format('d/m/Y H:i') }}
        </div>
    </div>
