@extends('layouts.app', ['title' => 'Detail Transaksi ' . $penjualan->nomor_transaksi])
...
<style>
    ...
    .rx-back { font-size:0.78rem; color:#10b981; text-decoration:none; font-weight:700; display:inline-flex; align-items:center; gap:4px; margin-bottom:1rem; }
    ...
    .rx-number { font-family:'JetBrains Mono',monospace; font-size:0.85rem; font-weight:700; color:#059669; background:#d1fae5; display:inline-block; padding:3px 10px; border-radius:8px; margin-top:0.5rem; }
    .rx-status { display:inline-block; padding:0.3rem 0.85rem; border-radius:10px; font-size:0.75rem; font-weight:700; background:#d1fae5; color:#059669; margin-top:0.5rem; }
    ...
    .rx-table .mono { font-family:'JetBrains Mono',monospace; font-weight:700; color:#059669; }
    ...
    .rx-total-value { font-size:1.2rem; font-weight:800; color:#059669; }

    .rx-pay { display:inline-block; padding:0.25rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; }
    .rx-pay.tunai { background:#dbeafe; color:#1d4ed8; }
    .rx-pay.transfer { background:#fef3c7; color:#92400e; }
    ...
    .rx-btn-outline { background:#fff; color:#059669; border:1.5px solid #10b981; }
    .rx-btn-outline:hover { background:#f0fdf4; }
    .rx-btn-print { background:linear-gradient(135deg,#10b981,#059669); color:#fff; box-shadow:0 2px 8px rgba(16,185,129,0.2); }
    .rx-btn-print:hover { box-shadow:0 4px 16px rgba(16,185,129,0.3); }
</style>
@endpush

@section('content')
<div class="rx-page">
    <a href="{{ route('pasgar.penjualan.index') }}" class="rx-back">
    ...
    <div class="rx-header">
        ...
        <p>{{ $penjualan->tanggal->format('d/m/Y H:i') }}</p>
        <div class="rx-number">{{ $penjualan->nomor_transaksi }}</div>
        <br>
        <span class="rx-status">✓ Transaksi Selesai</span>
    </div>
    ...
    <div class="rx-card">
        <div class="rx-info-grid">
            ...
            <div>
                <div class="rx-info-label">Loading</div>
                <div class="rx-info-value">{{ $penjualan->loading->nomor_loading ?? '-' }}</div>
            </div>
            <div>
                <div class="rx-info-label">Pelanggan</div>
                <div class="rx-info-value">
                    @if($penjualan->pelanggan_id && $penjualan->pelanggan)
                        {{ $penjualan->pelanggan->nama_toko }}
                    @else
                        {{ $penjualan->nama_pelanggan ?? 'Umum' }}
                    @endif
                </div>
            </div>
            <div>
                <div class="rx-info-label">Pembayaran</div>
                <div class="rx-info-value"><span class="rx-pay {{ $penjualan->metode_bayar }}">{{ strtoupper($penjualan->metode_bayar) }}</span></div>
            </div>
        </div>
        ...
        <div class="rx-transfer-box">
            ...
            <div class="rx-tf-row">
                ...
                <div class="rx-tf-item">ID Transaksi: <span>{{ $penjualan->id_transaksi_transfer }}</span></div>
                @endif
            </div>
        ...
        @if($penjualan->telepon_pelanggan)
            <div style="margin-top:0.5rem;">
                <div class="rx-info-label">No HP</div>
                <div class="rx-info-value">{{ $penjualan->telepon_pelanggan }}</div>
            </div>
        @endif
        @if($penjualan->alamat_pelanggan)
            <div style="margin-top:0.5rem;">
                <div class="rx-info-label">Alamat</div>
                <div class="rx-info-value">{{ $penjualan->alamat_pelanggan }}</div>
            </div>
        @endif
    </div>
                ...
                <tr>
                    <td>
                        ...
                        @if($item->unitConversion)
                            <div style="font-size:0.68rem;color:#94a3b8;">{{ $item->unitConversion->unit->name ?? '' }}</div>
                        @endif
                    </td>
                    <td class="num">{{ $item->qty }}</td>
                    <td class="num mono">{{ number_format($item->harga, 0, ',', '.') }}</td>
                    <td class="num mono">{{ number_format($item->subtotal, 0, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="rx-card">
        ...
        <div style="font-size:0.8rem;color:#475569;margin-top:0.25rem;">{{ $penjualan->catatan }}</div>
    </div>
    @endif

    <div class="rx-actions">
        <a href="{{ route('pasgar.penjualan.index') }}" class="rx-btn rx-btn-outline">← Daftar Penjualan</a>
        @if(Route::has('pasgar.penjualan.print'))
        <a href="{{ route('pasgar.penjualan.print', $penjualan->id) }}" class="rx-btn rx-btn-print">
