...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .search-input:focus{border-color:var(--accent);background:#fff;box-shadow:0 0 0 3px rgba(59,130,246,0.1)}
        ...
        .product-card:hover{border-color:var(--accent);box-shadow:0 4px 12px rgba(59,130,246,0.08);transform:translateY(-1px)}
        ...
        .config-select:focus{border-color:var(--accent);background-color:#fff;box-shadow:0 0 0 3px rgba(59,130,246,0.08)}
        ...
        .btn-pay:hover{background:var(--accent-dark);transform:translateY(-1px);box-shadow:0 4px 12px rgba(59,130,246,0.25)}
        ...
        .modal-total-box{background:linear-gradient(135deg,#eff6ff,#dbeafe);border-radius:var(--radius);padding:1rem;text-align:center;margin-bottom:1rem;border:1px solid #bfdbfe}
        .modal-total-label{font-size:0.68rem;color:#1e40af;font-weight:700;text-transform:uppercase;letter-spacing:0.06em}
        .modal-total-amount{font-size:1.85rem;font-weight:900;color:#1e3a8a;font-family:ui-monospace,monospace;margin-top:4px}
        ...
        .form-input:focus{border-color:var(--accent);background:#fff;box-shadow:0 0 0 3px rgba(59,130,246,0.1)}
        ...
        /* ── BANK DISPLAY ── */
        .bank-info{background:var(--accent-light);border:1px solid #bfdbfe;border-radius:10px;padding:0.75rem;margin-bottom:0.75rem}
        .bank-info-title{font-weight:800;color:#1e3a8a;font-size:0.72rem;margin-bottom:3px}
        ...
        /* ── VEHICLE SECTION ── */
        .vehicle-section{margin-top:0.875rem;padding-top:0.875rem;border-top:1px solid var(--border)}
        .vehicle-section .form-label{margin-bottom:4px}
        .vehicle-grid{display:grid;grid-template-columns:1fr 1fr;gap:0.5rem}
    ...
    <div class="modal">
        ...
        <div class="vehicle-section">
            ...
            <div class="vehicle-grid">
                <select id="vehicleId" class="config-select">
                    <option value="">-- Kendaraan --</option>
                    @foreach($vehicles as $v)
                        <option value="{{ $v->id }}">{{ $v->license_plate }} {{ $v->type ? '('.$v->type.')' : '' }}</option>
                    @endforeach
                </select>
                <input type="text" class="form-input" id="driverName" placeholder="Nama supir" style="font-size:0.82rem">
            </div>
        </div>
...
function printFaktur() { if(lastTransactionId) window.open('/print/faktur-grosir/' + lastTransactionId, '_blank'); }
...
function fetchProducts(query = '') {
    const url = '{{ route("kasir.grosir.search_products") }}' + '?_t=' + Date.now() + (query ? '&q=' + encodeURIComponent(query) : '');
...
document.getElementById('priceTier')?.addEventListener('change', function(){ priceTier = this.value || 'grosir'; refreshCartPrices(); });
...
function render(){
    ...
    grid.innerHTML = list.map(p => {
        const unitRows = p.units.length
            ...
            : `<div class="prod-unit-row"><span style="color:var(--muted)">-</span><span class="prod-unit-price">Rp ${fmt(p.price)}</span></div>`;
...
function addToCart(id){
    ...
    if(ex) {
        ...
        if(newQty * u.factor > wh.qty) return alert(`Stok di gudang ini (${wh.qty} base unit) tidak cukup! Sudah ada ${ex.qty} ${ex.unitName} di keranjang.`);
        ex.qty = newQty;
    } else {
        ...
        cart.push({key, id, name:p.name, units:p.units, unitId:u.id, unitName:u.name, factor:u.factor, price:unitPrice, originalPrice:unitPrice, purchasePrice: u.purchase_price || 0, minimalPrice: u.minimal_price || 0, qty:1, warehouse_id:wh.warehouse_id, breakdowns:p.stock_breakdown});
    }
    renderCart();
}
...
function changeUnit(i, uid){
    ...
    if(u) {
        ...
        else { item.unitId=u.id; item.unitName=u.name; item.factor=u.factor; item.price=resolveUnitPrice(u); item.originalPrice=item.price; item.purchasePrice=u.purchase_price||0; item.minimalPrice=u.minimal_price||0; item.key=`${item.id}_${u.id}_${item.warehouse_id}`; }
    }
    renderCart();
}
...
function doPayment(){
    ...
    const payload = {
        ...
        customer_id: custId ? custId : null, vehicle_id: vehicleId, driver_name: driverName
    };
    const btn = document.getElementById('btnConfirm'); btn.disabled = true; btn.textContent = 'Memproses...';
    fetch('{{ route("kasir.grosir.store") }}', {
        method: 'POST',
