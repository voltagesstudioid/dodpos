...
<head>
    ...
    <title>Kasir Grosir — DODPOS</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        body{
            ...
            background: radial-gradient(900px 380px at 18% 10%, rgba(59,130,246,0.14), transparent 60%),
                        radial-gradient(900px 380px at 85% 22%, rgba(16,185,129,0.10), transparent 55%),
                        var(--bg);
            color:var(--text);
            height:100vh;
            display:flex;
        ...
        .pos-badge{background:linear-gradient(135deg,var(--accent),var(--accent-600));padding:.3rem .875rem;border-radius:999px;font-size:.75rem;font-weight:900;color:#fff;letter-spacing:.04em}
        .topbar-title{font-size:1rem;font-weight:900;color:var(--text);letter-spacing:-0.02em}
        .topbar-pill{font-size:.72rem;color:#1d4ed8;background:#eff6ff;border:1px solid #bfdbfe;padding:.22rem .6rem;border-radius:999px;font-weight:800}
        ...
        .search-input:focus{border-color:rgba(59,130,246,0.65);outline:3px solid rgba(59,130,246,0.12)}
        ...
        .cat-btn.active,.cat-btn:hover{background:linear-gradient(135deg,var(--accent),var(--accent-600));border-color:transparent;color:#fff}
        ...
        .product-card:hover{border-color:rgba(59,130,246,0.55);transform:translateY(-1px);box-shadow:0 22px 60px rgba(15,23,42,0.10)}
        ...
        .prod-unit-name{color:#1d4ed8;font-weight:800}
        .prod-unit-price{color:var(--accent-600);font-weight:900}
        ...
        .cart-count{background:linear-gradient(135deg,var(--accent),var(--accent-600));color:#fff;border-radius:999px;padding:.12rem .5rem;font-size:.7rem;font-weight:900}
        ...
        .customer-select:focus{border-color:rgba(59,130,246,0.65);outline:3px solid rgba(59,130,246,0.12)}
        ...
        .unit-select:focus{border-color:rgba(59,130,246,0.65);outline:3px solid rgba(59,130,246,0.12)}
        ...
        .qty-btn:hover{background:#eff6ff;border-color:#bfdbfe;color:#1d4ed8}
        .qty-num{min-width:24px;text-align:center;font-weight:900;font-size:.85rem}
        .ci-subtotal{font-size:.8rem;font-weight:900;color:var(--accent-600);white-space:nowrap}
        ...
        .btn-pay{width:100%;background:linear-gradient(135deg,var(--accent),var(--accent-600));border:none;color:#fff;padding:.9rem;border-radius:14px;font-size:1rem;font-weight:900;cursor:pointer;font-family:inherit;transition:all .2s;box-shadow:0 18px 44px rgba(59,130,246,0.20)}
        .btn-pay:hover{transform:translateY(-1px);box-shadow:0 22px 60px rgba(59,130,246,0.26)}
        ...
        .modal-total-amount{font-size:1.9rem;font-weight:900;color:var(--accent-600);letter-spacing:-0.02em}
        ...
        .form-input:focus{border-color:rgba(59,130,246,0.65);outline:3px solid rgba(59,130,246,0.12)}
        ...
        .pay-method.active{border-color:rgba(59,130,246,0.65);color:var(--accent-600);background:#eff6ff}
        ...
        .btn-confirm{flex:2;background:linear-gradient(135deg,var(--accent),var(--accent-600));border:none;color:#fff;padding:.9rem;border-radius:14px;font-size:.95rem;font-weight:900;cursor:pointer;font-family:inherit}
        ...
        .btn-new{background:linear-gradient(135deg,var(--accent),var(--accent-600));border:none;color:#fff;padding:.8rem 1.1rem;border-radius:14px;font-size:.9rem;font-weight:900;cursor:pointer;font-family:inherit;margin-top:0.75rem;width:100%}
...
<div class="topbar">
    <div class="topbar-left">
        ...
        <span class="pos-badge">GROSIR</span>
        <span class="topbar-title">Kasir Grosir</span>
        <span class="topbar-pill">Semua satuan & harga tersedia</span>
    </div>
...
<div class="pos-layout">
    <div class="product-panel">
        ...
        <!-- Category Bar removed for AJAX simplicity -->
...
<div class="modal-overlay" id="payModal">
    <div class="modal">
        <div class="modal-title">💳 Pembayaran Grosir</div>
...
<div class="modal-overlay" id="successOverlay">
    <div class="success-box">
        ...
        <div style="font-size:1.25rem;font-weight:800;color:#3b82f6;margin-bottom:.25rem">Transaksi Selesai!</div>
        ...
        <button class="btn-new" id="btnCetakFaktur" style="background:#1d4ed8; margin-bottom: 0.5rem;" onclick="printFaktur()">🖨️ Cetak Faktur Grosir (3 Lembar)</button>
...
function printFaktur() {
    if(lastTransactionId) {
        window.open('/print/faktur-grosir/' + lastTransactionId, '_blank');
    }
}
...
let priceTier = 'grosir';
// const cats = [...new Set(PRODUCTS.map(p=>p.category).filter(Boolean))].sort(); // Deprecated
...
function render(){
    ...
    grid.innerHTML = PRODUCTS.map(p => `
        <div class="product-card ${p.stock <= 0 ? 'prod-nostock' : ''}" onclick="addToCart(${p.id})">
            ...
            <div class="prod-price">Rp ${fmt(p.price)}</div>
            <div class="prod-unit">/ ${p.units?.[0]?.name || 'pcs'}</div>
        </div>
    `).join('');
}
...
function addToCart(id){
    ...
    // Default unit (usually smallest or base)
    const defUnit = p.units.find(u => u.is_base) || p.units[0];
    
    const exist = cart.find(x => x.id === id && x.unitId == defUnit.id);
    if(exist) {
        ...
        cart.push({
            id: p.id, 
            name: p.name, 
            price: resolveUnitPrice(defUnit), 
            qty: 1, 
            max: p.stock, 
            unitId: defUnit.id,
            unitName: defUnit.name,
            units: p.units
        });
    }
    renderCart();
...
function render(){
    ...
    grid.innerHTML=list.map(p=>{
        // Show all unit prices on the card (the distinctive grosir feature)
        const unitRows = p.units.length
            ...
            : `<div class="prod-unit-row"><span style="color:#64748b">-</span><span class="prod-unit-price">Rp ${fmt(p.price)}</span></div>`;
...
// Cart — grosir: full unit selector in cart, multiple warehouses available
function addToCart(id){
    const p=PRODUCTS.find(x=>x.id===id);
    if(!p||p.stock<=0) return;
    const u=p.units[0]||{id:0,name:'-',factor:1,prices:{eceran:p.price}};
    const unitPrice = resolveUnitPrice(u);
    
    let wh = null;
    if (p.stock_breakdown && p.stock_breakdown.length > 0) {
        wh = p.stock_breakdown.find(b => b.qty >= u.factor);
        if (!wh) wh = p.stock_breakdown[0];
    } else {
        wh = {warehouse_id: null, qty: 1};
    }
    
    const key=`${id}_${u.id}_${wh.warehouse_id}`;
    const ex=cart.find(c=>c.key===key);
    
    if(ex) {
        if(ex.qty * u.factor + 1 * u.factor > wh.qty) return alert(`Stok di gudang ini (${wh.qty} base unit) tidak cukup!`);
        ex.qty++;
    } else {
        if(1 * u.factor > wh.qty) return alert(`Stok di gudang ini (${wh.qty} base unit) tidak cukup untuk satuan ${u.name} (butuh ${u.factor} base unit)!`);
        cart.push({
            key,
            id,
            name:p.name,
...
function changeWarehouse(i, wh_id){
    ...
    if(newWh) {
        if (item.qty * item.factor > newWh.qty) {
            ...
            if (item.qty === 0) {
                alert(`Tidak dapat menggunakan satuan ${item.unitName} karena stok base unit kurang dari ${item.factor}. Item akan dihapus.`);
                removeItem(i);
                return;
            }
        }
...
function changeUnit(i,uid){
    const item=cart[i]; const u=item.units.find(x=>x.id==uid);
    if(u){
        ...
        if (wh && (item.qty * u.factor > wh.qty)) {
            ...
            item.unitId=u.id;item.unitName=u.name;item.factor=u.factor;item.price=resolveUnitPrice(u);item.key=`${item.id}_${u.id}_${item.warehouse_id}`;
        }
    }
    renderCart();
}
...
function changeQty(i,d){
    ...
    if (d > 0) {
        const wh = item.breakdowns.find(b => b.warehouse_id == item.warehouse_id);
        if (wh && (newQty * item.factor > wh.qty)) {
            alert(`Stok di gudang ini (${wh.qty} base unit) tidak cukup!`);
            return;
        }
    }
    item.qty = newQty;
        ...
        <div class="cart-item">
            ...
            <div class="ci-bottom">
                <select class="unit-select" onchange="changeUnit(${i},this.value)">
                    ${c.units.map(u=>`<option value="${u.id}" ${u.id==c.unitId?'selected':''}>${u.name} — Rp ${fmt(resolveUnitPrice(u))}</option>`).join('')}
                </select>
                <div class="qty-ctrl">
...
function openPay(){
    ...
    const steps=[...new Set([total,Math.ceil(total/10000)*10000,Math.ceil(total/50000)*50000,Math.ceil(total/100000)*100000,Math.ceil(total/500000)*500000])].slice(0,4);
...
function calcChange(){
    const total=getTotal(), paid=parseFloat(document.getElementById('paidInput').value)||0, ch=paid-total;
    document.getElementById('changeDisplay').textContent='Rp '+fmt(Math.max(0,ch));
