...
function addToCart(id){
    ...
    if(ex) {
        ...
        cart.push({key, id, name:p.name, units:p.units, unitId:u.id, unitName:u.name, factor:u.factor, price:unitPrice, qty:1, warehouse_id:wh.warehouse_id, breakdowns:p.stock_breakdown});
    }
    renderCart();
}
...
function changeUnit(i, uid){
    ...
    if(u) {
        ...
        else { item.unitId=u.id; item.unitName=u.name; item.factor=u.factor; item.price=resolveUnitPrice(u); item.key=`${item.id}_${u.id}_${item.warehouse_id}`; }
    }
    renderCart();
}
