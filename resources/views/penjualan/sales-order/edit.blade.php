<x-app-layout>
    ...
    <style>
        ...
        .so-pg{background:linear-gradient(135deg,#fffbeb 0%,#f8fafc 50%,#fef3c7 100%);min-height:calc(100vh - 64px);padding:2rem 1.5rem;font-family:'Plus Jakarta Sans',system-ui,sans-serif;}
        ...
        .so-badges{display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;}
        ...
        .so-status-pill{display:inline-flex;align-items:center;gap:5px;font-size:.72rem;font-weight:700;padding:5px 12px;border-radius:20px;}
        .so-status-pill::before{content:'';width:6px;height:6px;border-radius:50%;}
        .so-sp-draft{background:#fef3c7;color:#92400e;}.so-sp-draft::before{background:#d97706;}
        .so-sp-confirmed{background:#dbeafe;color:#1e40af;}.so-sp-confirmed::before{background:#2563eb;}
        .so-sp-processing{background:#e0e7ff;color:#3730a3;}.so-sp-processing::before{background:#6366f1;}
        .so-sp-completed{background:#d1fae5;color:#065f46;}.so-sp-completed::before{background:#059669;}
        .so-sp-cancelled{background:#ffe4e6;color:#9f1239;}.so-sp-cancelled::before{background:#e11d48;}
        ...
        .so-inp:focus{border-color:#f59e0b;background:#fff;box-shadow:0 0 0 3px rgba(245,158,11,.1);}
        ...
        .so-item:hover{border-color:#fcd34d;background:#fffdf7;}
        .so-item-top{display:flex;align-items:center;gap:.75rem;}
        .so-item-num{width:28px;height:28px;border-radius:8px;background:#fef3c7;color:#92400e;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:800;flex-shrink:0;}
        ...
        .so-item-sub{font-weight:800;color:#d97706;font-size:.95rem;white-space:nowrap;}
        ...
        .so-empty-hint{display:inline-flex;align-items:center;gap:6px;margin-top:.75rem;font-size:.72rem;font-weight:700;color:#d97706;background:#fef3c7;padding:6px 14px;border-radius:8px;}

        .so-add-btn{display:flex;align-items:center;justify-content:center;gap:8px;padding:.85rem;border:2px dashed #fcd34d;border-radius:12px;background:transparent;color:#d97706;font-weight:700;font-size:.85rem;cursor:pointer;transition:.2s;font-family:inherit;width:100%;}
        .so-add-btn:hover{background:#fffbeb;border-color:#f59e0b;}
        .so-add-btn kbd{background:#fef3c7;padding:2px 6px;border-radius:4px;font-size:.7rem;font-weight:800;}
        ...
        .so-sum-val.grand{font-size:1.2rem;color:#d97706;}
        ...
        .so-modal-hdr{padding:1.25rem 1.5rem;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;background:linear-gradient(135deg,#fffbeb,#fef3c7);}
        ...
        .so-search-inp:focus{border-color:#f59e0b;background-color:#fff;box-shadow:0 0 0 3px rgba(245,158,11,.1);}
        ...
        .so-result:hover{background:#fffbeb;}
        ...
        .so-result-price{font-weight:800;color:#d97706;font-size:.88rem;}
        ...
        /* ── delivery info badge ── */
        .so-dlv-info{display:inline-flex;align-items:center;gap:5px;font-size:.72rem;font-weight:700;padding:4px 10px;border-radius:7px;margin-top:4px;}
        .so-dlv-today{background:#dbeafe;color:#1e40af;}
        .so-dlv-tomorrow{background:#fef3c7;color:#92400e;}
        .so-dlv-overdue{background:#fee2e2;color:#b91c1c;}
        .so-dlv-future{background:#f0fdf4;color:#065f46;}
        ...
        <div class="so-wrap">
            ...
            <div class="so-title-bar">
                ...
                <div class="so-badges">
                    ...
                    <span class="so-status-pill so-sp-{{ $salesOrder->status }}">{{ ucfirst($salesOrder->status) }}</span>
                </div>
            </div>
            ...
            <form action="{{ route('sales-order.update', $salesOrder->id) }}" method="POST" id="soForm">
                ...
                <div class="so-layout">
                    ...
                    <div class="so-main">
                                    ...
                                    <div class="so-fld">
                                        ...
                                        @if($salesOrder->delivery_date)
                                            @php
                                                ...
                                                if ($dlvDate->isSameDay($today)) {
                                                    $dlvClass = 'so-dlv-today'; $dlvLabel = 'Kirim Hari Ini';
                                                } elseif ($dlvDate->isSameDay($tomorrow)) {
                                                    $dlvClass = 'so-dlv-tomorrow'; $dlvLabel = 'Kirim Besok';
                                                } elseif ($dlvDate->lt($today)) {
                                                    $dlvClass = 'so-dlv-overdue'; $dlvLabel = 'Terlambat ' . $dlvDate->diffInDays($today) . ' hari';
                                                } else {
                                                    $dlvClass = 'so-dlv-future'; $dlvLabel = $dlvDate->diffInDays($today) . ' hari lagi';
                                                }
                                            @endphp
                                            <span class="so-dlv-info {{ $dlvClass }}">{{ $dlvLabel }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="so-fld">
                        ...
                        <div class="so-card">
                            <div class="so-card-hdr">
                                ...
                                <button type="button" class="so-add-btn" style="width:auto;padding:.5rem 1rem;border:1.5px solid #fcd34d;border-radius:10px;" onclick="openProductModal()">
                                ...
                                <div class="so-items" id="itemsContainer">
                                    <div id="emptyState" class="so-empty">
                                        ...
                                        <div class="so-empty-hint">
                                            ...
                                            Tekan <kbd style="background:#fcd34d;padding:1px 5px;border-radius:3px;margin:0 2px;">/</kbd> untuk cari barang
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="so-summary">
                        ...
                        {{-- Order Meta Info --}}
                        <div class="so-sum-card" style="margin-top:1rem;">
                            <div class="so-sum-hdr">
                                <div class="so-card-icon blue" style="width:34px;height:34px;border-radius:9px;">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                                </div>
                                <div class="so-sum-title">Informasi</div>
                            </div>
                            <div class="so-sum-body">
                                <div class="so-sum-row">
                                    <span class="so-sum-lbl">Dibuat</span>
                                    <span class="so-sum-val" style="font-size:.78rem;">{{ $salesOrder->created_at ? $salesOrder->created_at->format('d M Y, H:i') : '-' }}</span>
                                </div>
                                <div class="so-sum-row">
                                    <span class="so-sum-lbl">Terakhir Diubah</span>
                                    <span class="so-sum-val" style="font-size:.78rem;">{{ $salesOrder->updated_at ? $salesOrder->updated_at->diffForHumans() : '-' }}</span>
                                </div>
                                @if($salesOrder->user)
                                <div class="so-sum-row">
                                    <span class="so-sum-lbl">Dibuat Oleh</span>
                                    <span class="so-sum-val" style="font-size:.78rem;">{{ $salesOrder->user->name ?? '-' }}</span>
                                </div>
                                @endif
                            </div>
                        </div>
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('input', function(e) {
            ...
            rc.innerHTML = '<div class="so-search-empty" style="color:#d97706;font-weight:600;">Mencari...</div>';
