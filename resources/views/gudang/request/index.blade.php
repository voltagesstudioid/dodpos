                ...
                <table class="pr-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="c">
                                @php
                                    $unitName = $req->unit?->abbreviation ?? $req->unit?->name ?? '';
                                    $baseUnitName = $req->product?->unit?->abbreviation ?? $req->product?->unit?->name ?? '';
                                    $hasConversion = $req->unit_id && $req->conversion_factor && (float)$req->conversion_factor > 1;
                                    $baseQty = $hasConversion ? (int) round((float)$req->quantity * (float)$req->conversion_factor) : null;
                                @endphp
                                <span class="pr-qty">
                                    {{ number_format((float)$req->quantity, 0) }}
                                    <span class="pr-qty-unit">{{ $unitName ?: ($baseUnitName ?: '') }}</span>
                                </span>
                                @if($hasConversion)
                                <div class="pr-qty-base">= {{ number_format($baseQty, 0) }} {{ $baseUnitName }}</div>
                                @endif
                            </td>
                            <td>
                                @if($req->type === 'po')
