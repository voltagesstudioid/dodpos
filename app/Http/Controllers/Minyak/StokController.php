...
class StokController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->isSales()) {
            $profile = MinyakSales::where('user_id', Auth::id())->first();
            $salesIds = $profile ? [$profile->id] : [];
            $sales = collect([$profile])->filter();
        } else {
            $query = MinyakSales::aktif();
        ...
        $loadings = MinyakLoading::with(['produk', 'sales.vehicle'])
        ...
        return view('minyak.stok.index', compact('stokPerSales', 'sales', 'isSalesRole'));
    }
}
