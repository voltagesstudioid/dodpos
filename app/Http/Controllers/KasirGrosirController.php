...
class KasirGrosirController extends Controller
{
    ...
    private function formatProduct($p)
    {
        ...
        return [
            ...
            'price' => $base ? (float) ($base['prices']['grosir'] ?? 0) : (float) $p->price,
            'units' => $units,
        ];
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            // Harga TIDAK divalidasi dari request — diambil dari DB sisi server
        ...
        try {
            ...
            foreach ($request->items as $item) {
                ...
                // Akumulasi stok terpakai (dalam satuan base) untuk produk dan gudang yang sama
                ...
                if ($conversion) {
                    $unitPrice = match ($priceTier) {
                        ...
                        default => ($conversion->sell_price_grosir > 0 ? $conversion->sell_price_grosir : $conversion->sell_price_ecer),
                    };
                }
            ...
            if ($request->payment_method === 'kredit') {
                ...
                if ($debtAmount > 0) {
                    \App\Models\CustomerCredit::create([
                        ...
                        'paid_amount' => 0,
                        'status' => 'unpaid',
                        'description' => 'Pembelian POS Grosir - #'.$trx->id,
                        'created_by' => Auth::id(),
                    ]);
            ...
            return response()->json(['success' => false, 'message' => $safeMessage], 500);
        }
    }
    ...
    private function deductWarehouseStock(int $productId, int $warehouseId, int $qty, int $transactionId, string $productName): void
    {
        ...
        foreach ($stocks as $stock) {
            ...
            StockMovement::create([
                ...
                'reference_number' => 'POS-GROSIR-'.$transactionId,
                'batch_number' => $stock->batch_number,
                'expired_date' => $stock->expired_date,
                'quantity' => $deduct,
                'balance' => $stock->stock,
                'notes' => '[POS Grosir] Transaksi #'.$transactionId,
                'user_id' => Auth::id(),
            ]);
