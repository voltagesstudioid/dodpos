...
class StokMasukController extends Controller
{
    public function index(Request $request)
    {
        ...
        $records = GulaStokMasuk::with(['produk', 'creator'])
        ...
        $stats = [
            'total_penerimaan' => GulaStokMasuk::where('tipe', 'penerimaan')->where('status', 'aktif')->sum('jumlah'),
            'total_koreksi' => GulaStokMasuk::where('tipe', 'koreksi')->where('status', 'aktif')->sum('jumlah'),
            'bulan_ini' => GulaStokMasuk::where('status', 'aktif')
                ->whereMonth('created_at', now()->month)
                ->sum('jumlah'),
        ];

        return view('gula.stok-masuk.index', compact('records', 'produks', 'stats'));
    }

    public function create()
    {
        $produks = GulaProduk::where('status', 'aktif')->orderBy('nama')->get();
        return view('gula.stok-masuk.create', compact('produks'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $record = GulaStokMasuk::create([
                'no_referensi' => GulaStokMasuk::generateReferensi($validated['tipe']),
            ...
            AuditService::log('gula_stok_masuk.create', 'GulaStokMasuk', $record->id, [
            ...
            return redirect()->route('gula.stok-masuk.index')
    ...
    public function show(GulaStokMasuk $stokMasuk)
    {
        $stokMasuk->load(['produk', 'creator']);
        return view('gula.stok-masuk.show', compact('stokMasuk'));
    }

    /**
     * Cancel: reverse the stock change.
     */
    public function destroy(GulaStokMasuk $stokMasuk)
    {
        ...
        DB::beginTransaction();
        try {
            $produk = GulaProduk::find($stokMasuk->produk_id);
            if ($produk) {
                if ($stokMasuk->tipe === 'penerimaan') {
                    ...
                    // Koreksi reversal: user should create a new koreksi to fix the stock
                }
                $produk->save();
            }
            ...
            AuditService::log('gula_stok_masuk.cancel', 'GulaStokMasuk', $stokMasuk->id, [
