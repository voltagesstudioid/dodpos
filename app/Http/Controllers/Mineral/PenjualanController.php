...
class PenjualanController extends Controller
{
     ...
     */
    private function updateLoadingStatus(MineralLoading $loading): void
    {
    ...
    public function index(Request $request)
    {
        ...
        $query = MineralPenjualan::with(['sales', 'pelanggan', 'produk']);
        ...
        $baseQuery = MineralPenjualan::query();
        ...
        return view('mineral.penjualan.index', compact('penjualans', 'sales', 'stats', 'isSalesRole'));
    }
    ...
    public function create()
    {
        ...
        // Build regional price map: salesId => [produkId => harga]
        $regionalPriceMap = [];
        foreach ($sales as $s) {
            if ($s && $s->regional_id) {
                $hargaItems = MineralRegionalHarga::where('regional_id', $s->regional_id)->get();
        ...
        return view('mineral.penjualan.create', compact('sales', 'pelanggans', 'produks', 'isSalesRole', 'regionalPriceMap'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Apply regional pricing if harga_satuan matches default product price
        $salesProfile = MineralSales::find($validated['sales_id']);
        if ($salesProfile && $salesProfile->regional_id) {
            $regionalHarga = MineralRegionalHarga::where('regional_id', $salesProfile->regional_id)
        ...
        // Validate loading stock availability
        ...
        if ($loading->sisa_stok < $validated['jumlah']) {
            return redirect()->back()->withInput()
                ->with('error', 'Stok di kendaraan tidak cukup. Sisa stok: ' . number_format($loading->sisa_stok, 0, ',', '.') . ' ' . ($loading->produk->satuan ?? 'unit'));
        }

        $validated['no_faktur'] = MineralPenjualan::generateFaktur();
        ...
        try {
            ...
            $penjualan = MineralPenjualan::create($validated);

            // Deduct from loading stock (already validated)
            ...
            if ($validated['hutang'] > 0) {
                ...
                $pelanggan = $pelanggan ?? MineralPelanggan::find($validated['pelanggan_id']);
                $pelanggan->total_hutang = (float) $pelanggan->total_hutang + (float) $validated['hutang'];
                $pelanggan->save();
            }

            // Audit log
            AuditService::log('mineral_penjualan.create', 'MineralPenjualan', $penjualan->id, [
            ...
            return redirect()->route('mineral.penjualan.index')
    ...
    public function show(MineralPenjualan $penjualan)
    {
        if ($this->isSales()) {
            $profile = $this->getSalesProfile();
        ...
        $penjualan->load(['sales', 'pelanggan', 'produk', 'verifier', 'hutang.pembayarans']);
        $isSalesRole = $this->isSales();

        return view('mineral.penjualan.show', compact('penjualan', 'isSalesRole'));
    }

    public function edit(MineralPenjualan $penjualan)
    {
        ...
        if ($penjualan->status !== 'pending') {
            return redirect()->route('mineral.penjualan.show', $penjualan)
        ...
        return view('mineral.penjualan.edit', compact('penjualan', 'sales', 'pelanggans', 'produks', 'isSalesRole'));
    }

    public function update(Request $request, MineralPenjualan $penjualan)
    {
        // Sales can only edit own transactions
        if ($this->isSales()) {
            $profile = $this->getSalesProfile();
        ...
        try {
            ...
            if ($newLoading->sisa_stok < $newJumlah) {
                DB::rollBack();
                return redirect()->back()->withInput()
                    ->with('error', 'Stok di kendaraan tidak cukup. Sisa: ' . number_format($newLoading->sisa_stok, 0, ',', '.') . ' ' . ($newLoading->produk->satuan ?? 'unit'));
            }

            // Deduct from new loading
            ...
            if ($oldHutang > 0) {
                $existingHutang = MineralHutang::where('penjualan_id', $penjualan->id)->first();
                if ($newHutang > 0 && $existingHutang) {
                ...
                } elseif ($existingHutang) {
                    // Hutang removed
                    $pelanggan = MineralPelanggan::find($penjualan->pelanggan_id);
                    if ($pelanggan) {
            ...
            AuditService::log('mineral_penjualan.update', 'MineralPenjualan', $penjualan->id, [
    ...
    public function verify(MineralPenjualan $penjualan)
    {
        ...
        AuditService::log('mineral_penjualan.verify', 'MineralPenjualan', $penjualan->id, [
            'no_faktur' => $penjualan->no_faktur,
    ...
    public function printStruk(MineralPenjualan $penjualan)
    {
        if ($this->isSales()) {
            $profile = $this->getSalesProfile();
            if ($profile && $penjualan->sales_id !== $profile->id) {
        ...
        return view('mineral.penjualan.print', compact('penjualan'));
    }
     ...
     */
    public function destroy(MineralPenjualan $penjualan)
    {
        ...
        DB::beginTransaction();
        try {
            // Return stock to the correct loading record
            ...
            if ($penjualan->kunjungan_id) {
                MineralKunjungan::where('id', $penjualan->kunjungan_id)
                    ->update(['ada_penjualan' => false]);
            }

            AuditService::log('mineral_penjualan.cancel', 'MineralPenjualan', $penjualan->id, [
                'no_faktur' => $penjualan->no_faktur,
